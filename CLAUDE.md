...
patchbay is a multi-transport chat orchestrator for the official provider CLIs (`claude`, `codex`, `gemini`, `agy`, `grok`).
It runs Telegram, Matrix and/or Slack, can expose an optional direct WebSocket API, keeps state under `~/.phoenix-patchbay`, and supervises the main agent plus optional sub-agents in one asyncio process.
...
| `workspace/` | `~/.phoenix-patchbay` path model, seeding, rule deployment/sync, skill sync |
...
- `PatchbayPaths` in `workspace/paths.py` is the single source of truth for runtime paths.
...
- Skill sync spans `~/.phoenix-patchbay/workspace/skills`, `~/.claude/skills`, `~/.codex/skills`, `~/.gemini/skills`, `~/.grok/skills`:
  - normal mode: links/junctions
  - Docker mode: managed copies (`.phoenix-patchbay_managed`)
...
- `patchbay service logs` behavior:
...
- `patchbay`, `patchbay onboarding`, `patchbay reset`
- `patchbay status`, `patchbay stop`, `patchbay restart`, `patchbay upgrade`, `patchbay uninstall`
