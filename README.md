...
- **9router** — point patchbay at a [9router](https://github.com/decolua/9router) instance and `/model` shows a `9ROUTER` button (in the slot Gemini used to occupy; Gemini models still switch with `/model <id>`). It runs the Claude Code CLI against the router, so every provider and combo 9router exposes becomes a model you can pick per chat or topic. Configure the address with environment variables or `~/.phoenix-patchbay/.env` (no config file edit, no restart for `.env` changes), and the key in `/settings`:
  - `NINEROUTER_BASE_URL` — e.g. `http://localhost:20128` (a trailing `/v1` is accepted)
  - API key: paste the key from the 9router dashboard into `/settings` → **API keys** → **9router**, which checks it against the router before saving (the same flow as the Gemini key). There is no environment variable for it
  - `NINEROUTER_MODELS` — optional comma list (`cc/claude-sonnet-4-5,premium-coding`); without it the button lists whatever `GET /v1/models` returns

  Models are addressed as `9router/<model>`, so `/model 9router/cc/claude-sonnet-4-5` works too. Reasoning effort is not forwarded — the router decides. Docker sandbox mode (`docker_container`) does not pass the router variables into the sandbox yet
- **Image generation**: `patchbay image "<prompt>" --out <path> [--model M] [--size WxH] [--overwrite]` lets any agent generate an image through an OpenAI-compatible images API (OpenAI, 9router, Together…), whatever model the agent itself runs on. Set `IMAGEGEN_BASE_URL` (OpenAI style, including `/v1`) and `IMAGEGEN_MODEL` in the environment or `~/.phoenix-patchbay/.env`, and the key in `/settings` → **API keys** → **Image generation**. The key is read from the config on every call and never enters the agent's environment. Use a dedicated key with a spending cap set at the provider. The key check asks the provider's `/models`, so a provider that lists models without authentication accepts any key there; the first real generation tells you. The command prints the saved path (the suffix follows the returned format) and never replaces a file without `--overwrite`. Agents send the image to you for a visual check rather than opening it themselves, unless you tell them to look. Not available inside Docker sandbox mode (`docker_container`)
...
- **Handoffs, written as you go** — every conversation keeps one in its project's `handoffs/`, excluded via `.git/info/exclude` and verified with `git check-ignore` on every write. What was asked is recorded by code on each turn; the write-up — objective, state, decisions, dead ends, what is next, each claim carrying a path or a commit — happens once a few turns' worth of work has piled up and you have not already sent the next message. `/compact` and `/clear` write one first too; `/handoff` shows the current one; `/clear` archives it outside the folder rather than deleting it. **Changing model or persona compacts as well**, because a different provider resumes nothing and a different persona inherits a transcript written by someone else — but not at the tap: a picker is also how a mistap gets corrected, so the write-up is spent by the next message you actually send, and browsing models costs nothing. Alongside the written-up sections, an injected handoff carries the newest raw log lines, so a task still in progress is described even when no write-up has run yet
...
- **The write-up can run on Gemini instead of your coding subscription** — paste a key into `/settings` (this Gemini key is only for handoff write-ups — it never runs your tasks; to chat with Gemini models, pick them under `9ROUTER` in `/model`, which use the separate 9router key) (free from [AI Studio](https://aistudio.google.com/apikey), or `GEMINI_API_KEY` for a fresh deployment) and the handoff is written by a separate Gemini call rather than by resuming the session that did the work. Measured at ~$0.17 a time on the session's own subscription, which is real money on a busy day and invisible in the per-session totals. The writer was not in the conversation, so it is handed the transcript since the last write-up with tool *results* stripped — decisions, not the output of every grep. It returns the document and patchbay writes it, so an answer that is not a handoff leaves the good one alone. **URLs and non-Latin strings never reach the model at all** — they are lifted out, replaced with plain ASCII placeholders, and substituted back verbatim, because a model asked to retype a URL will eventually drop a character from it. Once it did: an Arabic slug came back one letter short, a 404 in a document someone would later trust. The model arranges the handoff; it does not transcribe the literals. The slice it is handed is watermarked per session id, not per conversation: every compaction, provider switch and daily reset starts a new transcript, and a byte count only means something inside the file it was measured in — applied to the next transcript it silently skipped the beginning of it. A write-up that fails says why, with the provider's exit code and output, rather than logging an empty line. The model is pinned to `gemini-3.5-flash-lite` and is deliberately not configurable — the write-up has a fixed shape, so which model does it is an implementation detail, and at ~$0.009 a time against ~$0.17 in-session the choice is already 19× cheaper without asking anyone to tune it. It runs in the shared workspace rather than your project folder: it reads no files, so a summariser with tool access to a repository is blast radius nobody asked for. Verified end to end — three turns of real work, then a write-up naming the file, the rate change and the self-check that proves it. **When Gemini is unavailable — no key, a revoked one, a denied project, an outage — the write-up falls back to Claude Haiku, still off-session.** That keeps both properties worth having: it is not billed at coding-model prices, and its output never joins the context it is summarising. Resuming the session to write itself up is the last resort, for when no CLI can produce the document at all
- **`/settings`, so a preference never needs an SSH session** — values that used to mean editing JSON on the host as root are set from the chat. Its top screen holds **Model**, **Claude accounts** (the `/model` and `/account` screens, moved out of the main menu) and **API keys**, which lists **Gemini — handoff writer** and **9router — model access**. Each row carries its own state (`✅ set` / `⚠️ not set`) with the cost of leaving it unset written next to it, and a secret is masked to `AIza••••••••4f2` — never echoed in full. **A key is checked against the service before it is stored**, so `✅` means Google accepted it, not that it matched a pattern: a spent quota says so rather than claiming the key is bad, and "could not reach Google" never counts as success. A stored key keeps a `🔄 Test` button, because keys get revoked without any screen changing. Your message is deleted the moment it is read — before validation, since a mistyped secret is still a secret — though it does still pass through Telegram to get there, which the prompt says before you paste
...
| `/settings` | Model, Claude accounts, and API keys — the Gemini key for handoff write-ups (not for tasks) and the 9router key for the 9ROUTER models, each verified before it is stored |
