...
- **9router** — point patchbay at a [9router](https://github.com/decolua/9router) instance and `/model` shows a `9ROUTER` button (in the slot Gemini used to occupy; Gemini models still switch with `/model <id>`). It runs the Claude Code CLI against the router, so every provider and combo 9router exposes becomes a model you can pick per chat or topic. Configure it with environment variables or `~/.phoenix-patchbay/.env` — no config file edit, no restart for `.env` changes:
  ...
  - `NINEROUTER_API_KEY` — the key from the 9router dashboard. Easier: paste it into `/settings` → **9router API key**, which checks it against the router before saving (the same flow as the Gemini key) and takes precedence over the variable
...
| Command | Description |
|---|---|
| `/model` | Interactive model/provider selector — Claude aliases (`opus`, `sonnet`, `[1m]` for the 1M-context beta) plus pinned versions such as `OPUS 4.8`, and `9ROUTER` when `NINEROUTER_BASE_URL` is set |
