...
patchbay supports multiple messaging transports. The `transport` config
...
### `patchbay` entry (`phoenix_patchbay/__main__.py`)
...
1. resolve paths and set `PATCHBAY_HOME` for main agent
...
- shared registry: `~/.phoenix-patchbay/tasks.json`
- folders: `~/.phoenix-patchbay/workspace/tasks/<task_id>/`
...
- task tools receive `PATCHBAY_CHAT_ID` and optional `PATCHBAY_TOPIC_ID`
...
Source: `phoenix_patchbay/_home_defaults/`.
