...
- persisted in `~/.phoenix-patchbay/tasks.json`
- task folders in `~/.phoenix-patchbay/workspace/tasks/<task_id>/`
...
- forum-topic tasks route back to the originating topic via `thread_id` / `PATCHBAY_TOPIC_ID`

## Cron jobs

Cron jobs run in `~/.phoenix-patchbay/workspace/cron_tasks/<task_folder>/`.
...
```text
~/.phoenix-patchbay/workspace/cron_tasks/weather-report/
  CLAUDE.md
  AGENTS.md
  TASK_DESCRIPTION.md
