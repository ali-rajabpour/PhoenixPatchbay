...
4. One of these messaging transports:
    ...
    - **Matrix**: install Matrix support first (`patchbay install matrix` or `pip install \"patchbay[matrix]\"`), then provide homeserver URL, user ID, and password/access token
    - **Slack**: install Slack support first (`pip install "patchbay[slack]"`), then create a Slack app with Socket Mode, the bot/app scopes below, and provide bot/app tokens plus Slack member/channel IDs for the allowlist
...
- writes config and seeds `~/.phoenix-patchbay/`.
...
patchbay's Slack transport uses **Slack Bolt + Socket Mode** — no public webhook URL is needed.

```bash
pip install "patchbay[slack]"   # or: patchbay install slack
```
...
- On Linux, patchbay maps UID/GID to avoid root-owned files.
- If Docker setup fails at startup, patchbay logs warning and falls back to host execution.

Docker CLI shortcuts:

```bash
patchbay docker enable
patchbay docker disable
patchbay docker rebuild
patchbay docker mount /path/to/project
patchbay docker unmount /path/to/project
patchbay docker mounts
patchbay docker extras
patchbay docker extras-add <id>
patchbay docker extras-remove <id>
```
...
- run `patchbay docker mounts` to inspect effective mapping and broken paths.
...
- after changing extras, run `patchbay docker rebuild` to apply. Build output is streamed live to the terminal.
...
`patchbay api enable` requires PyNaCl (used for E2E encryption against the direct API). PyNaCl is **only needed when the direct WebSocket API is enabled** — the core bot, Telegram, and Matrix transports run without it. If it is missing:

```bash
# pipx install
pipx inject patchbay PyNaCl

# pip install
pip install "patchbay[api]"
```
...
- token is generated and persisted by `patchbay api enable` (runtime also generates it on API start if still empty).
...
```bash
patchbay service install
```

Manage:

```bash
patchbay service status
patchbay service start
patchbay service stop
...
- Linux: `systemd --user` service `~/.config/systemd/user/patchbay.service`
- macOS: Launch Agent `~/Library/LaunchAgents/dev.phoenix-patchbay.plist`
- Windows: Task Scheduler task `patchbay`
...
- service install prefers `pythonw.exe -m phoenix_patchbay` (no visible console window),
...
- Linux: live `journalctl --user -u patchbay -f`
- macOS/Windows: recent lines from `~/.phoenix-patchbay/logs/agent.log` (fallback newest `*.log`)
...
- use `/upgrade` or `pipx upgrade patchbay`
...
2. run `patchbay status`
3. inspect `~/.phoenix-patchbay/logs/agent.log`
...
```bash
pipx upgrade patchbay
```

Uninstall:

```bash
pipx uninstall patchbay
rm -rf ~/.phoenix-patchbay  # optional data removal
```
