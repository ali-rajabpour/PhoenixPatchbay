...
patchbay's primary transport is Telegram. Matrix is an optional second transport you can add at any time — either as the only transport or running alongside Telegram.
...
```bash
# pipx (recommended)
patchbay install matrix

# pip
pip install "patchbay[matrix]"
...
| User ID | `@my_phoenix_patchbay:matrix.org` |
...
Edit `~/.phoenix-patchbay/config/config.json`:
...
```json
{
  ...
  "matrix": {
    ...
    "user_id": "@my_phoenix_patchbay:matrix.org",
...
On first start, patchbay logs in with the password and saves credentials to `~/.phoenix-patchbay/matrix_store/credentials.json`. After that, the password is no longer needed — token-based auth is used automatically.
...
| `store_path` | no | Relative to patchbay_home. Default: `matrix_store` |
...
| Sub-agent setup | `patchbay agents add` (interactive) | Manual via `agents.json` |
...
- Run `patchbay install matrix` to install the dependency
...
- Delete `~/.phoenix-patchbay/matrix_store/credentials.json` and restart — patchbay will re-login with password
