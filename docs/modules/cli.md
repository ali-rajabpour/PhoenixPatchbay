...
- `antigravity_cache.py`, `antigravity_cache_observer.py`: Antigravity model cache + observer (refreshes `~/.phoenix-patchbay/config/antigravity_models.json` from `agy models`, hourly)
...
- `PATCHBAY_CHAT_ID`
- `PATCHBAY_TOPIC_ID` (when set)
- `PATCHBAY_TRANSPORT` (active transport identifier, e.g. `"tg"`, `"mx"`)
- `PATCHBAY_AGENT_NAME`
- `PATCHBAY_INTERAGENT_PORT`
- `PATCHBAY_HOME`
- `PATCHBAY_SHARED_MEMORY_PATH`
- `PATCHBAY_TRANSCRIBE_COMMAND` / `PATCHBAY_VIDEO_TRANSCRIBE_COMMAND` when external transcription hooks are configured

Host subprocesses additionally get:

- `PATCHBAY_AGENT_ROLE` (`main` or `sub`)
...
- an `--append-system-prompt` value over 96 KiB would exceed the kernel per-argument limit (`execve` E2BIG), so it is written to a temp file and passed via `--append-system-prompt-file` instead (cleaned up after the run; in Docker mode the container-side path under the `/patchbay` mount is passed)
...
- may inject `GEMINI_API_KEY` from patchbay config when Gemini settings indicate API-key mode and no env key is set
...
- Gemini:
  ...
  - optional fallback to `~/.phoenix-patchbay/config/config.json` `gemini_api_key`
...
- file: `~/.phoenix-patchbay/config/codex_models.json`
...
- file: `~/.phoenix-patchbay/config/gemini_models.json`
...
- file: `~/.phoenix-patchbay/config/antigravity_models.json`
...
- file: `~/.phoenix-patchbay/config/grok_models.json` (per-agent home, e.g. `~/.phoenix-patchbay-cto/config/`)
...
- container mode:
  ...
  - injects `PATCHBAY_CHAT_ID`, optional `PATCHBAY_TOPIC_ID`, `PATCHBAY_TRANSPORT`, `PATCHBAY_AGENT_NAME`, `PATCHBAY_INTERAGENT_PORT`, `PATCHBAY_HOME`, `PATCHBAY_SHARED_MEMORY_PATH`, `PATCHBAY_INTERAGENT_HOST`, and optional transcription hook vars,
  - merges user secrets from `~/.phoenix-patchbay/.env` (never overrides existing vars),
