...
- `phoenix_patchbay/config_reload.py`: `ConfigReloader`, config diff/classification helpers

## Purpose

Avoid unnecessary restarts when safe `config.json` fields change.

`ConfigReloader` polls `~/.phoenix-patchbay/config/config.json` every 5 seconds, validates with `AgentConfig`, diffs top-level schema fields, and:
...
- environment/core: `patchbay_home`, `log_level`, `gemini_api_key`, `update_check`
