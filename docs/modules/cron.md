...
File: `~/.phoenix-patchbay/cron_jobs.json`
...
Opt-in (`cron_preflight.enabled=false` by default). Before the agent subprocess is built, `CronObserver._run_preflight(...)` runs `cron_tasks/<task_folder>/scripts/preflight.py` with `sys.executable` in the task folder (`PATCHBAY_HOME` exported).
...
- `PATCHBAY_CHAT_ID` -- current chat ID (set when `chat_id` is non-zero)
- `PATCHBAY_TOPIC_ID` -- current topic ID (set when `topic_id` is non-None)
- `PATCHBAY_TRANSPORT` -- transport identifier (`"tg"` or `"mx"`)
