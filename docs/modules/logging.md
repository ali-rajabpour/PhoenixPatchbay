...
- `phoenix_patchbay/log_context.py`: `ContextVar` state (`agent_name`, `operation`, `chat_id`, `session_id`) + `ContextFilter`.
- `phoenix_patchbay/logging_config.py`: root logger setup (console + rotating file via queue listener).
...
- rotating file logs in `~/.phoenix-patchbay/logs/agent.log` (`5MB`, `3` backups)
