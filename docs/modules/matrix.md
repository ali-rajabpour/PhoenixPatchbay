...
Matrix can be configured via the interactive setup wizard (`patchbay onboarding`) or manually in `config.json`.
...
- `patchbay agents add <name>` currently scaffolds Telegram sub-agents only
...
1. On first start, the bot logs in with the `password` and saves credentials to `~/.phoenix-patchbay/<store_path>/credentials.json` (mode `0o600`)
