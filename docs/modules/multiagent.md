...
Multi-agent runtime: run multiple independent patchbay agents in one process.
...
- workspace and files under `~/.phoenix-patchbay/agents/<name>/`
...
- central log file (`~/.phoenix-patchbay/logs/agent.log`)
- shared knowledge source (`~/.phoenix-patchbay/SHAREDMEMORY.md`)
...
`SharedKnowledgeSync` watches `~/.phoenix-patchbay/SHAREDMEMORY.md` and mirrors content into each agent's `MAINMEMORY.md` block.
...
- `patchbay agents`
- `patchbay agents list`
- `patchbay agents add <name>`
- `patchbay agents remove <name>`

`patchbay agents list` fetches live health from internal API when main bot is running.

Important CLI nuance:

- `patchbay agents add <name>` currently prompts for Telegram token/user/group data only.
