...
Cross-tool skill sync between patchbay workspace and CLI skill homes.
...
- main: `~/.phoenix-patchbay`
- sub-agent: `~/.phoenix-patchbay/agents/<name>`

## Bundled skills

Bundled source: `phoenix_patchbay/_home_defaults/workspace/skills/`.
...
2. union names
3. pick canonical source by priority (`patchbay > claude > codex > gemini > grok`)
...
Toggles are read live from `config.json` on each sync tick, so changes take effect within one interval without restart. Disabling a provider stops new links from being recreated; existing patchbay-created links remain until shutdown cleanup or manual removal.
...
`cleanup_patchbay_links(paths)` removes managed links under CLI skill dirs whose targets point to managed roots.
