...
- sub-agents are loaded from `~/.phoenix-patchbay/agents.json`
...
3. if `tasks.enabled=true`: create shared `TaskHub` (`~/.phoenix-patchbay/tasks.json` + `~/.phoenix-patchbay/workspace/tasks/`) and attach it to `InternalAgentAPI`
