...
patchbay's primary transport is Telegram. Slack is an optional transport you can add at any time — either as the only transport or running alongside Telegram and/or Matrix. The Slack transport uses **Slack Bolt + Socket Mode**, so no public webhook URL is needed.
...
```bash
# pipx (recommended)
patchbay install slack

# pip
pip install "patchbay[slack]"
...
patchbay does not register native Slack slash commands. Its command keywords work as normal messages (for example `help`, `status`, or `model`) and also accept a leading `/`.
...
- Run `patchbay install slack` to install the dependency
