...
Fastest end-to-end mental model for patchbay.
...
- optional sub-agent stacks from `~/.phoenix-patchbay/agents.json`
...
## 7) Key runtime files (`~/.phoenix-patchbay`)
...
Sub-agent home: `~/.phoenix-patchbay/agents/<name>/` with its own config/workspace/session files.

## 8) Where to read code first

1. `phoenix_patchbay/__main__.py` (entrypoint + config/load/run)
2. `phoenix_patchbay/cli_commands/` (actual CLI subcommand logic)
3. `phoenix_patchbay/multiagent/supervisor.py` (always-on runtime wrapper)
4. `phoenix_patchbay/messenger/telegram/app.py` + `messenger/telegram/startup.py` (Telegram), `phoenix_patchbay/messenger/matrix/bot.py` (Matrix)
5. `phoenix_patchbay/orchestrator/core.py` + `orchestrator/lifecycle.py`
6. `phoenix_patchbay/bus/*` (unified delivery/injection)
7. `phoenix_patchbay/tasks/hub.py` + `tasks/registry.py`
8. `phoenix_patchbay/cli/service.py` and provider wrappers
...
- `patchbay`
- `patchbay status|stop|restart|upgrade|uninstall|onboarding|reset|help`
- `patchbay service ...`
- `patchbay docker ...` (includes `extras`, `extras-add`, `extras-remove` for optional AI/ML packages)
- `patchbay api ...`
- `patchbay agents ...` (`add` currently scaffolds Telegram sub-agents; Matrix sub-agents are added via `agents.json` or tool scripts)
- `patchbay install <extra>` (`matrix`, `api`)
...
- `patchbay service logs` follows `journalctl` on Linux and tails file logs on macOS/Windows
