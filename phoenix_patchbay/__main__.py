"""Entry point: python -m phoenix_patchbay."""
...
# tests that patch phoenix_patchbay.__main__.<name>.
...
from phoenix_patchbay.cli_commands.service import cmd_service as _cmd_service
from phoenix_patchbay.cli_commands.status import (
...
def load_config() -> AgentConfig:
    ...
    Resolution order:
    1. ``~/.phoenix-patchbay/config/config.json`` (canonical location)
    ...
    if first_start:
        ...
        if example.is_file():
            ...
            defaults.pop("api", None)  # Beta: only written by `patchbay api enable`
...
async def run_bot(config: AgentConfig) -> int:
    ...
    """
    paths = resolve_paths(patchbay_home=config.patchbay_home)
    _validate_transports(config)

    from phoenix_patchbay.infra.pidlock import acquire_lock, release_lock
    from phoenix_patchbay.multiagent.supervisor import AgentSupervisor

    acquire_lock(pid_file=paths.patchbay_home / "bot.pid", kill_existing=True)
    ...
    try:
        ...
        release_lock(pid_file=paths.patchbay_home / "bot.pid")
    return exit_code
...
def _cmd_setup(verbose: bool) -> None:
    ...
    from phoenix_patchbay.cli.init_wizard import run_onboarding, run_smart_reset

    _stop_bot()
    paths = resolve_paths()
    if _is_configured():
        run_smart_reset(paths.patchbay_home)
...
def _default_action(verbose: bool) -> None:
    ...
    if not _is_configured():
        from phoenix_patchbay.cli.init_wizard import run_onboarding
...
def main() -> None:
    ...
    if "--version" in args or "-V" in args:
        from phoenix_patchbay import __version__

        _console.print(f"patchbay {__version__}")
        return
