...
Environment variables PATCHBAY_AGENT_NAME, PATCHBAY_INTERAGENT_PORT, and
PATCHBAY_INTERAGENT_HOST are automatically set by the Patchbay framework.
...
Options:
    ...
    --reply-to AGENT     Route the response to AGENT instead of the sending agent.
                         ...
                         PATCHBAY_AGENT_NAME falls back to "unknown"). The async result
...
def main() -> None:
    ...
    target = args[0]
    message = args[1]
    port = os.environ.get("PATCHBAY_INTERAGENT_PORT", "8799")
    host = os.environ.get("PATCHBAY_INTERAGENT_HOST", "127.0.0.1")
    sender = os.environ.get("PATCHBAY_AGENT_NAME", "unknown")
    ...
    if silent:
        body["silent"] = True
    chat_id = os.environ.get("PATCHBAY_CHAT_ID", "")
    topic_id = os.environ.get("PATCHBAY_TOPIC_ID", "")
    transport = os.environ.get("PATCHBAY_TRANSPORT", "")
    if chat_id:
    ...
    try:
        ...
        print(
            "Make sure the Patchbay supervisor is running with multi-agent support.", file=sys.stderr
        )
        sys.exit(1)
