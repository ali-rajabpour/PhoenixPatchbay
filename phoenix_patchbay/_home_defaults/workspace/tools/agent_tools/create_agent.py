...
def _agents_path() -> Path:
    ...
    Sub-agents have PATCHBAY_HOME = ~/.phoenix-patchbay/agents/<name>/, so we navigate
    up to the main home. Main agent's PATCHBAY_HOME points directly to ~/.phoenix-patchbay/.
    """
    import os

    home = Path(os.environ.get("PATCHBAY_HOME", str(Path.home() / ".phoenix-patchbay")))
...
def _main_home() -> Path:
    """Resolve the main agent's PATCHBAY_HOME."""
    import os

    home = Path(os.environ.get("PATCHBAY_HOME", str(Path.home() / ".phoenix-patchbay")))
    if home.name != ".phoenix-patchbay" and (home.parent.parent / "config").is_dir():
