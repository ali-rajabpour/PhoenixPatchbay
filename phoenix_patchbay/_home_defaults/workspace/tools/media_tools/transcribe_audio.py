...
def _transcribe_external(path: Path) -> dict:
    """Transcribe via the ``PATCHBAY_TRANSCRIBE_COMMAND`` external hook (#66).
    ...
    """
    raw = os.environ.get("PATCHBAY_TRANSCRIBE_COMMAND", "").strip()
    if not raw:
        return {"error": "PATCHBAY_TRANSCRIBE_COMMAND unset"}

    try:
        argv = shlex.split(raw)
    except ValueError as exc:
        return {"error": f"Invalid PATCHBAY_TRANSCRIBE_COMMAND: {exc}"}
    if not argv:
        return {"error": "PATCHBAY_TRANSCRIBE_COMMAND empty after split"}
