...
from phoenix_patchbay.cli.antigravity_events import parse_antigravity_json
from phoenix_patchbay.cli.antigravity_runtime import antigravity_process_env
from phoenix_patchbay.cli.base import BaseCLI, CLIConfig
from phoenix_patchbay.cli.executor import build_subprocess_env
from phoenix_patchbay.cli.process_registry import ProcessRegistry, TrackedProcess
from phoenix_patchbay.cli.stream_events import AssistantTextDelta, ResultEvent, StreamEvent
from phoenix_patchbay.cli.types import CLIResponse
from phoenix_patchbay.config import ANTIGRAVITY_MODELS
...
from phoenix_patchbay.workspace.skill_sync import _create_dir_link
...
class AntigravityCLI(BaseCLI):
    ...
    def _agy_workspace(self) -> Path:
        ...
        ~/.phoenix-patchbay/workspace) and falls back to its scratch sandbox, so the
    ...
    def _build_command(
        ...
        *,
        ...
        # Ground agy in patchbay's per-agent workspace so its tools operate there
...
# so patchbay's ~/.phoenix-patchbay/workspace is never accepted. agy checks the literal path
...
def _agy_state_root(env: Mapping[str, str] | None = None) -> Path:
    ...
    environment handed to the agy subprocess so patchbay reads exactly where agy
