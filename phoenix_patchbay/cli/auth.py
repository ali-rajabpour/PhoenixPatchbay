...
from phoenix_patchbay.cli.gemini_utils import find_gemini_cli
...
def _gemini_key_auth_source(gemini_home: Path) -> tuple[Path | None, datetime | None]:
    ...
    patchbay_key, patchbay_config_path = read_patchbay_gemini_api_key()
    if patchbay_key and patchbay_config_path is not None:
        return patchbay_config_path, datetime.fromtimestamp(
            patchbay_config_path.stat().st_mtime, tz=UTC
        )
    return None, None
...
def read_patchbay_gemini_api_key() -> tuple[str | None, Path | None]:
    """Read ``gemini_api_key`` from ``~/.phoenix-patchbay/config/config.json``.

    Returns ``(key, path)`` when configured, otherwise ``(None, None)``.
    """
    config_path = _patchbay_config_path()
...
def _patchbay_config_path() -> Path:
