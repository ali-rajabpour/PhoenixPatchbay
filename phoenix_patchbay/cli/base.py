...
from phoenix_patchbay.cli._log_redact import redact_cmd_for_log
from phoenix_patchbay.cli.stream_events import StreamEvent
...
class CLIConfig:
    ...
    persona: str = ""
    # A complete settings.json document for --settings, used to narrow which
    # plugins this run loads. Empty means none is passed and the CLI reads its
    # own configuration; see phoenix_patchbay.personas.plugin_scope.
    settings_json: str = ""
...
_CONTAINER_PATCHBAY_MOUNT = "/patchbay"


def _to_container_path(host_path: Path, main_home: Path) -> str:
    """Map a host path under *main_home* to its container equivalent.

    The Docker container mounts the root patchbay home at ``/patchbay``.
    """
    rel = host_path.relative_to(main_home)
    if str(rel) == ".":
        return _CONTAINER_PATCHBAY_MOUNT
    return f"{_CONTAINER_PATCHBAY_MOUNT}/{rel.as_posix()}"
...
def _docker_env_flags(
    ...
    env_flags: list[str] = [
        "-e",
        f"PATCHBAY_CHAT_ID={config.chat_id}",
        "-e",
        f"PATCHBAY_TRANSPORT={config.transport}",
        "-e",
        f"PATCHBAY_AGENT_NAME={config.agent_name}",
        "-e",
        f"PATCHBAY_INTERAGENT_PORT={config.interagent_port}",
        "-e",
        f"PATCHBAY_HOME={container_home}",
        "-e",
        f"PATCHBAY_SHARED_MEMORY_PATH={container_shared}",
        "-e",
        "PATCHBAY_INTERAGENT_HOST=host.docker.internal",
    ]
    if config.topic_id:
        env_flags += ["-e", f"PATCHBAY_TOPIC_ID={config.topic_id}"]
    if task_id := task_id_from_label(config.process_label):
        env_flags += ["-e", f"PATCHBAY_TASK_ID={task_id}"]
    if config.transcribe_command:
        env_flags += ["-e", f"PATCHBAY_TRANSCRIBE_COMMAND={config.transcribe_command}"]
    if config.video_transcribe_command:
        env_flags += [
            "-e",
            f"PATCHBAY_VIDEO_TRANSCRIBE_COMMAND={config.video_transcribe_command}",
        ]
    return env_flags
...
def docker_wrap(
    ...
    *extra_env* vars are injected as ``-e`` flags into ``docker exec``
    ...
    if config.docker_container:
        ...
        patchbay_home = working_dir.parent if working_dir.name == "workspace" else working_dir

        # Resolve root patchbay home for host → container path mapping.
        # Sub-agents live at <root>/agents/<name>/; the Docker mount is the root.
        main_home = patchbay_home
        if main_home.parent.name == "agents":
            main_home = main_home.parent.parent

        container_cwd = _to_container_path(working_dir, main_home)
        container_home = _to_container_path(patchbay_home, main_home)
...
def host_path_to_container(host_path: str) -> str | None:
    """Translate a host path under the patchbay home to its container path.

    The Docker container bind-mounts the patchbay home at ``/patchbay``.  Returns
    ...
    prefix = str(resolve_paths().patchbay_home)
    if host_path.startswith(prefix):
        return _CONTAINER_PATCHBAY_MOUNT + host_path[len(prefix) :].replace("\\", "/")
    return None
...
def docker_prompt_tmp_dir() -> str:
    ...
    Prompt files handed to a containerized CLI must live under the patchbay home
    so they are readable through the ``/patchbay`` mount.
    """
    from phoenix_patchbay.workspace.paths import resolve_paths

    tmp_dir = resolve_paths().patchbay_home / "tmp"
