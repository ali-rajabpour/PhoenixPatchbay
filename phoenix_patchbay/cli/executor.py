...
from phoenix_patchbay.cli.base import (
    _IS_WINDOWS,
    CLIConfig,
...
from phoenix_patchbay.infra.platform import CREATION_FLAGS as _CREATION_FLAGS
from phoenix_patchbay.infra.process_tree import force_kill_process_tree
...
def build_subprocess_env(config: CLIConfig) -> dict[str, str] | None:
    ...
    agent identification variables.  User secrets from ``~/.phoenix-patchbay/.env`` are
    ...
    env_file = patchbay_home / ".env"
    for key, value in load_env_secrets(env_file).items():
        if key not in env:
            env[key] = value

    env["PATCHBAY_AGENT_NAME"] = config.agent_name
    env["PATCHBAY_AGENT_ROLE"] = "main" if config.agent_name == "main" else "sub"
    env["PATCHBAY_INTERAGENT_PORT"] = str(config.interagent_port)
    if config.chat_id:
        env["PATCHBAY_CHAT_ID"] = str(config.chat_id)
    if config.topic_id:
        env["PATCHBAY_TOPIC_ID"] = str(config.topic_id)
    env["PATCHBAY_TRANSPORT"] = config.transport
    if task_id := task_id_from_label(config.process_label):
        env["PATCHBAY_TASK_ID"] = task_id
    if config.transcribe_command:
        env["PATCHBAY_TRANSCRIBE_COMMAND"] = config.transcribe_command
    if config.video_transcribe_command:
        env["PATCHBAY_VIDEO_TRANSCRIBE_COMMAND"] = config.video_transcribe_command
    working_dir = Path(config.working_dir)
    patchbay_home = working_dir.parent if working_dir.name == "workspace" else working_dir
    env["PATCHBAY_HOME"] = str(patchbay_home)
    # Shared knowledge is always at the main agent's home level.
    # For main: patchbay_home itself. For sub-agents: ../../ from agents/<name>/.
    if config.agent_name == "main":
        env["PATCHBAY_SHARED_MEMORY_PATH"] = str(patchbay_home / "SHAREDMEMORY.md")
    else:
        # Sub-agent home is <main_home>/agents/<name>/
        main_home = patchbay_home.parent.parent
        env["PATCHBAY_SHARED_MEMORY_PATH"] = str(main_home / "SHAREDMEMORY.md")
