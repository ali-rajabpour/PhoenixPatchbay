...
from phoenix_patchbay.cli.auth import gemini_api_key_mode_selected
...
from phoenix_patchbay.cli.gemini_events import (
    extract_result_text,
    extract_text,
    parse_gemini_stream_line,
)
from phoenix_patchbay.cli.gemini_utils import (
    create_system_prompt_file,
    find_gemini_cli,
    find_gemini_cli_js,
)
from phoenix_patchbay.cli.stream_events import ResultEvent, StreamEvent, SystemInitEvent
from phoenix_patchbay.cli.types import CLIResponse
from phoenix_patchbay.config import NULLISH_TEXT_VALUES
...
class GeminiCLI(BaseCLI):
    ...
    def _inject_config_gemini_api_key(self, env: dict[str, str]) -> None:
        """Inject GEMINI_API_KEY from patchbay config when API-key auth mode is active."""
        ...
        logger.debug("Injected GEMINI_API_KEY from patchbay config for Gemini API key mode")
    ...
    def _create_system_prompt_path(self) -> str | None:
        ...
        In Docker mode the file is written to ``~/.phoenix-patchbay/tmp/`` which is
