...
from phoenix_patchbay.cli.auth import (
    AuthResult,
    AuthStatus,
...
from phoenix_patchbay.config import DEFAULT_EMPTY_GEMINI_API_KEY, AgentConfig, deep_merge_config
from phoenix_patchbay.i18n import t_rich
from phoenix_patchbay.workspace.init import init_workspace
...
def _load_banner() -> str:
    ...
    try:
        ...
        return "github.com/ali-rajabpour/PhoenixPatchbay"
...
def _build_extras_table(console: Console) -> None:
    ...
    from phoenix_patchbay.infra.docker_extras import DOCKER_EXTRAS_BY_ID, extras_for_display
...
def _ask_docker_extras(console: Console) -> list[str]:
    ...
    from phoenix_patchbay.infra.docker_extras import (
        DOCKER_EXTRAS_BY_ID,
        extras_for_display,
        resolve_extras,
    )
...
def _offer_service_install(console: Console) -> bool:
    """Ask whether to install patchbay as a background service."""
    from phoenix_patchbay.infra.service import is_service_available
...
def run_onboarding() -> bool:
    ...
    console.print(
        Panel(
            ...
            + "\n\n"
            + t_rich("wizard.complete.home", path=paths.patchbay_home)
            + "\n"
    ...
    if run_as_service:
        from phoenix_patchbay.infra.service import install_service
...
def run_smart_reset(patchbay_home: Path) -> None:
    """Read existing config, handle Docker cleanup, and delete workspace."""
    console = Console()
    console.print()

    config_path = patchbay_home / "config" / "config.json"
    ...
    if config_path.exists():
        try:
            ...
            if isinstance(docker, dict) and docker.get("enabled"):
                docker_container = str(docker.get("container_name", "patchbay-sandbox"))
                docker_image = str(docker.get("image_name", "patchbay-sandbox"))
        except (json.JSONDecodeError, OSError):
            pass
    ...
    console.print(
        Panel(
            t_rich("wizard.reset.body", home=patchbay_home),
    ...
    if patchbay_home.exists():
        console.print(t_rich("wizard.reset.confirm.warning", home=patchbay_home) + "\n")
    else:
