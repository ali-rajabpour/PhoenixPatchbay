"""Agent management CLI subcommands (``patchbay agents ...``)."""
...
def _parse_agents_subcommand(args: list[str]) -> tuple[str | None, list[str]]:
    ...
    for a in args:
        ...
        if found and sub is not None:
            rest.append(a)
    if found and sub is None:
        # bare "patchbay agents" → default to list
        return "list", []
    return sub, rest
...
def print_agents_help() -> None:
    ...
    table.add_column()
    table.add_row("patchbay agents", "List all sub-agents and their config")
    table.add_row("patchbay agents list", "List all sub-agents and their config")
    table.add_row("patchbay agents add <name>", "Add a new sub-agent (interactive)")
    table.add_row("patchbay agents remove <name>", "Remove a sub-agent")
    _console.print(
...
def load_agents_registry(paths: PatchbayPaths) -> list[dict[str, object]]:
    """Load sub-agent definitions from agents.json (raw dicts)."""
    agents_path = paths.patchbay_home / "agents.json"
...
def fetch_live_health() -> dict[str, dict[str, object]]:
    ...
    from phoenix_patchbay.multiagent.internal_api import _DEFAULT_PORT
...
def validate_agent_name(name: str | None, agents: list[dict[str, object]]) -> str | None:
    """Validate an agent name for ``patchbay agents add``. Returns clean name or None on error."""
    if not name:
...
def agents_remove(rest: list[str]) -> None:
    ...
    _console.print(t_rich("agents.remove.data_hint", path=paths.patchbay_home / "agents" / name))


def cmd_agents(args: list[str]) -> None:
    """Handle 'patchbay agents [subcommand]'."""
