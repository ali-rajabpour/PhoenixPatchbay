"""API server management CLI subcommands (``patchbay api ...``)."""
...
from phoenix_patchbay.config import _BIND_ALL_INTERFACES
...
def print_api_help() -> None:
    ...
    table.add_column()
    table.add_row("patchbay api enable", "Enable the WebSocket API server")
    table.add_row("patchbay api disable", "Disable the WebSocket API server")
...
def api_install_hint() -> str:
    ...
    if mode == "pipx":
        return "pipx inject patchbay PyNaCl"
    return "pip install patchbay[api]"


def api_enable() -> None:
    """Enable the API server: check deps, write config, generate token."""
    from phoenix_patchbay.cli_commands.docker import docker_read_config
...
def cmd_api(args: list[str]) -> None:
    """Handle 'patchbay api <subcommand>'."""
