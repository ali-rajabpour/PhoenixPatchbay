"""Docker management CLI subcommands (``patchbay docker ...``)."""
...
def _parse_docker_mount_arg(args: list[str]) -> str | None:
    ...
    Expects the form: ``patchbay docker mount <path>`` where *args* is
    ``sys.argv[1:]`` (no ``patchbay`` prefix).  Non-flag positionals are
...
def print_docker_help() -> None:
    ...
    table.add_column()
    table.add_row("patchbay docker rebuild", "Remove container & image, rebuild on next start")
    table.add_row("patchbay docker enable", "Enable Docker sandboxing")
    table.add_row("patchbay docker disable", "Disable Docker sandboxing")
    table.add_row("patchbay docker mount <path>", "Mount a host directory into the sandbox")
    table.add_row("patchbay docker unmount <path>", "Remove a mounted directory")
    table.add_row("patchbay docker mounts", "List all mounted directories")
    table.add_row("patchbay docker extras", "List available and installed extras")
    table.add_row("patchbay docker extras-add <id>", "Add an extra package")
    table.add_row("patchbay docker extras-remove <id>", "Remove an extra package")
    _console.print(
...
def docker_set_enabled(*, enabled: bool) -> None:
    ...
    if not isinstance(docker, dict):
        data["docker"] = docker = {}
    from phoenix_patchbay.infra.json_store import atomic_json_save
...
def docker_rebuild() -> None:
    ...
    result = docker_read_config()
    container = "patchbay-sandbox"
    image = "patchbay-sandbox"
...
def docker_mount(args: list[str]) -> None:
    ...
    from phoenix_patchbay.infra.docker import resolve_mount_target
...
def docker_extras_list() -> None:
    ...
    from phoenix_patchbay.infra.docker_extras import extras_for_display
...
def docker_extras_add(args: list[str]) -> None:
    ...
    from phoenix_patchbay.infra.docker_extras import DOCKER_EXTRAS_BY_ID, resolve_extras
...
def docker_extras_remove(args: list[str]) -> None:
    ...
    from phoenix_patchbay.infra.docker_extras import DOCKER_EXTRAS, DOCKER_EXTRAS_BY_ID
...
def cmd_docker(args: list[str]) -> None:
    """Handle 'patchbay docker <subcommand>'."""
