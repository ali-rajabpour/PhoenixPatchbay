"""CLI command: patchbay install <extra>."""
...
def _install_extra(name: str) -> None:
    ...
    if mode == "pipx":
        cmd = [sys.executable, "-m", "pip", "install", f"patchbay[{name}]"]
    elif mode == "dev":
...
def print_install_help() -> None:
    ...
    Console().print(Panel(table, title="patchbay install <extra>"))


def cmd_install(args: list[str]) -> None:
    """Handle `patchbay install <extra>`."""
