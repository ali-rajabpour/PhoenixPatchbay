...
from phoenix_patchbay.cli_commands.docker import _stop_docker_container
from phoenix_patchbay.i18n import t_rich
from phoenix_patchbay.infra.fs import robust_rmtree
from phoenix_patchbay.infra.platform import is_windows
...
def _re_exec_bot() -> NoReturn:
    ...
    Spawns a new Python process running ``phoenix_patchbay`` and exits the current one.
    Under a service manager the caller should ``sys.exit(EXIT_RESTART)`` instead.
    """
    subprocess.Popen([sys.executable, "-m", "phoenix_patchbay"])
    sys.exit(0)
...
def _stop_service_if_running() -> None:
    ...
    with contextlib.suppress(Exception):
        from phoenix_patchbay.infra.service import (
...
def stop_bot() -> None:
    """Stop all running patchbay instances and Docker container.

    1. Stop the system service (prevents Task Scheduler/systemd/launchd respawn)
    2. Kill the PID-file instance
    3. Kill any remaining patchbay processes system-wide
    4. Wait for file locks to release (Windows only)
    5. Stop Docker container if enabled
    """
    from phoenix_patchbay.infra.pidlock import _is_process_alive, _kill_and_wait
    ...
    # 3. Kill all remaining patchbay processes system-wide
    from phoenix_patchbay.infra.process_tree import kill_all_patchbay_processes

    extra = kill_all_patchbay_processes()
    if extra:
    ...
    if config_path.exists():
        try:
            ...
            if isinstance(docker, dict) and docker.get("enabled"):
                container = str(docker.get("container_name", "patchbay-sandbox"))
...
def start_bot(verbose: bool = False) -> None:
    ...
    from phoenix_patchbay.__main__ import load_config, run_telegram
    from phoenix_patchbay.logging_config import setup_logging

    paths = resolve_paths()
    setup_logging(verbose=verbose, log_dir=paths.logs_dir)
    config = load_config()
    from phoenix_patchbay.i18n import init as init_i18n
    ...
    if exit_code == EXIT_RESTART:
        if os.environ.get("PATCHBAY_SUPERVISOR") or os.environ.get("INVOCATION_ID"):
...
def uninstall() -> None:
    ...
    # 1. Stop bot + Docker container + all patchbay processes
    stop_bot()
    ...
    if paths.config_path.exists():
        try:
            ...
            if isinstance(docker, dict) and docker.get("enabled") and shutil.which("docker"):
                image = str(docker.get("image_name", "patchbay-sandbox"))
    ...
    # 3. Delete workspace
    patchbay_home = paths.patchbay_home
    if patchbay_home.exists():
        robust_rmtree(patchbay_home)
        if patchbay_home.exists():
            _console.print(t_rich("lifecycle.uninstall.delete_warning", home=patchbay_home))
        else:
            _console.print(t_rich("lifecycle.uninstall.deleted", home=patchbay_home))
    ...
    if shutil.which("pipx"):
        ...
        subprocess.run(
            [sys.executable, "-m", "pip", "uninstall", "-y", "patchbay"],
            capture_output=True,
            check=False,
        )
