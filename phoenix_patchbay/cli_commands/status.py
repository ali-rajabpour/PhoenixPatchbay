"""Status display CLI commands (``patchbay status``, ``patchbay help``)."""
...
def build_status_lines(status: StatusSummary, *, paths: PatchbayPaths) -> list[str]:
    ...
    lines.append(f"  Home:       [cyan]{paths.patchbay_home}[/cyan]")
...
def print_status() -> None:
    ...
    from phoenix_patchbay.cli_commands.agents import load_agents_registry, print_agents_status
    ...
    if docker_enabled and isinstance(docker_cfg, dict):
        docker_name = str(docker_cfg.get("container_name", "patchbay-sandbox"))
    ...
    if pid_file.exists():
        ...
        if bot_pid is not None:
            from phoenix_patchbay.infra.pidlock import _is_process_alive
...
def print_usage() -> None:
    ...
    try:
        ...
        banner_text = "github.com/ali-rajabpour/PhoenixPatchbay"
    _console.print(
        Panel(
    ...
    table.add_column()
    table.add_row("patchbay", t_rich("help.phoenix-patchbay"))
    table.add_row("patchbay onboarding", t_rich("help.onboarding"))
    table.add_row("patchbay stop", t_rich("help.stop"))
    table.add_row("patchbay restart", t_rich("help.restart"))
    table.add_row("patchbay reset", t_rich("help.reset"))
    table.add_row("patchbay upgrade", t_rich("help.upgrade"))
    table.add_row("patchbay uninstall", t_rich("help.uninstall"))
    is_macos = sys.platform == "darwin"
    svc_hint = "Task Scheduler" if is_windows() else ("launchd" if is_macos else "systemd")
    table.add_row("patchbay service install", t_rich("help.service_install", hint=svc_hint))
    table.add_row("patchbay service", t_rich("help.service"))
    table.add_row("patchbay agents", t_rich("help.agents"))
    table.add_row("patchbay docker", t_rich("help.docker"))
    table.add_row("patchbay api", t_rich("help.api"))
    table.add_row("patchbay install <extra>", t_rich("help.install"))
    table.add_row("patchbay status", t_rich("help.status"))
    table.add_row("patchbay help", t_rich("help.help"))
