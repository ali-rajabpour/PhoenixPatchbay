...
from phoenix_patchbay.cli.claude_accounts import ENV_VAR as CLAUDE_ACCOUNT_ENV
from phoenix_patchbay.cli.codex_events import parse_codex_jsonl
from phoenix_patchbay.cli.gemini_events import parse_gemini_json
from phoenix_patchbay.cli.gemini_utils import find_gemini_cli
from phoenix_patchbay.cli.grok_events import parse_grok_json
...
def enrich_instruction(instruction: str, task_folder: str) -> str:
    ...
    return (
        ...
        "- The final answer is delivered to Telegram automatically by patchbay.\n"
...
def _build_grok_cmd(exec_config: CLIRunConfig, prompt: str) -> OneShotCommand | None:
    ...
    if len(prompt) > _GROK_PROMPT_ARGV_SOFT_LIMIT:
        handle = tempfile.NamedTemporaryFile(  # noqa: SIM115
            mode="w",
            encoding="utf-8",
            prefix="patchbay-grok-cron-prompt-",
            suffix=".txt",
            delete=False,
        )
        with handle:
