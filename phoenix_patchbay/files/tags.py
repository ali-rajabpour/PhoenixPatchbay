...
_DOCKER_MOUNT = "/patchbay"


def _resolve_container_path(path: Path) -> Path:
    """Translate Docker container paths to host paths.

    Inside the sandbox container ``~/.phoenix-patchbay`` is mounted at ``/patchbay``.
    CLI output references container-side paths like
    ``/patchbay/workspace/output_to_user/file.png`` which don't exist on the
