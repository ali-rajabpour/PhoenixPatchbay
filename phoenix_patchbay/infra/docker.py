...
_PATCHBAY_MOUNT = "/patchbay"
_CONTAINER_WS = f"{_PATCHBAY_MOUNT}/workspace"
_MOUNT_PREFIX = "/mnt"
...
class DockerManager:
    ...
    def __init__(self, config: DockerConfig, paths: PatchbayPaths) -> None:
        self._config = config
        self._paths = paths
    ...
    async def _setup_unlocked(self) -> str | None:
        ...
        if not await self._image_exists(image):
            ...
            if self._config.extras:
                from phoenix_patchbay.infra.docker_extras import DOCKER_EXTRAS_BY_ID
    ...
    async def _start_container(self, name: str, image: str) -> bool:
        # Always mount the root patchbay home, even when called from a sub-agent.
        ...
        # like /patchbay/agents/<name>/workspace.
        patchbay_home = self._paths.patchbay_home
        if patchbay_home.parent.name == "agents":
            patchbay_home = patchbay_home.parent.parent
        ...
        cmd: list[str] = [
            ...
            "-w",
            _CONTAINER_WS,
            # Mount the ENTIRE ~/.phoenix-patchbay so the CLI sees all framework files.
            "-v",
            f"{patchbay_home}:{_PATCHBAY_MOUNT}",
            "-e",
            f"PATCHBAY_HOME={_PATCHBAY_MOUNT}",
    ...
    def _env_secret_flags(self) -> list[str]:
        """Return ``-e`` flags for user secrets from ``~/.phoenix-patchbay/.env``."""
