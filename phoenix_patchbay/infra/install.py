"""Detect how patchbay was installed (pipx, uv tool, pip, or dev/source)."""
...
def detect_install_mode() -> InstallMode:
    ...
    Returns:
        ``"pipx"`` -- installed via ``pipx install patchbay``
        ``"uv"``   -- installed via ``uv tool install patchbay``
        ``"pip"``  -- installed via ``pip install patchbay`` (from PyPI)
