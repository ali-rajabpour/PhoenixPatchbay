...
def kill_all_patchbay_processes() -> int:
    """Find and force-kill remaining ``patchbay`` processes system-wide.

    On Windows this uses two strategies:
    1. ``tasklist`` scan for ``patchbay.exe`` processes
    2. PowerShell scan for ``python.exe``/``pythonw.exe`` running from the
       pipx venv directory (covers ``pythonw.exe -m phoenix_patchbay``)
    ...
    current = os.getpid()
    killed = _kill_patchbay_exe_windows(current)
    killed += _kill_venv_python_windows(current)
    return killed


def _kill_patchbay_exe_windows(current_pid: int) -> int:
    """Kill processes whose image name is ``patchbay.exe``."""
    try:
    ...
    for line in result.stdout.splitlines():
        ...
        if len(parts) < 2:
            continue
        name = parts[0].lower()
        if name not in ("patchbay.exe", "patchbay"):
            continue
        with contextlib.suppress(ValueError):
            pid = int(parts[1])
            if pid == current_pid or pid <= 0:
                continue
            logger.info("Killing patchbay process: pid=%d name=%s", pid, name)
            _run_taskkill(pid, force=True)
            killed += 1
    return killed
...
def _kill_venv_python_windows(current_pid: int) -> int:
    ...
    When patchbay is installed via ``pipx``, the bot runs as
    ``pythonw.exe -m phoenix_patchbay`` inside ``~\pipx\venvs\patchbay\``.
        ...
        result = subprocess.run(
            [
                ...
                (
                    ...
                    " | Where-Object { $_.Path -like '*pipx*patchbay*' }"
