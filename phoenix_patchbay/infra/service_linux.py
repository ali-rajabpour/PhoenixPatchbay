"""Systemd user service management for patchbay (Linux)."""
...
from phoenix_patchbay.infra.service_logs import print_journal_service_logs
...
def _generate_service_unit(binary_path: str) -> str:
    ...
    return f"""\
[Unit]
Description=patchbay - Telegram bot powered by AI CLIs
...
Environment=PATCHBAY_SUPERVISOR=1
...
def is_service_installed() -> bool:
    """Check if the patchbay service is installed."""
    return _service_path().exists()


def is_service_running() -> bool:
    """Check if the patchbay service is currently running."""
...
def install_service(console: Console | None = None) -> bool:
    """Install and start the patchbay systemd user service.
...
def uninstall_service(console: Console | None = None) -> bool:
    """Stop, disable, and remove the patchbay service."""
    console = ensure_console(console)
