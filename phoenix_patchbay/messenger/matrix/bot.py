...
from phoenix_patchbay.commands import BOT_COMMANDS, MULTIAGENT_SUB_COMMANDS
...
from phoenix_patchbay.messenger.matrix.buttons import ButtonTracker
from phoenix_patchbay.messenger.matrix.credentials import login_or_restore
from phoenix_patchbay.messenger.matrix.id_map import MatrixIdMap
from phoenix_patchbay.messenger.matrix.sender import send_rich as matrix_send_rich
from phoenix_patchbay.messenger.matrix.streaming import MatrixStreamEditor
from phoenix_patchbay.messenger.matrix.typing import MatrixTypingContext
...
def _expand_marker(patchbay_home: str) -> Path:
    """Return the restart-marker path (sync, no I/O)."""
    return Path(patchbay_home).expanduser() / "restart-requested"
...
class MatrixBot:
        ...
        *,
        ...
        try:
            ...
            raise ImportError(
                ...
                "Install with: pip install 'patchbay[matrix]'"
            ) from None

        self._config = config
        self._agent_name = agent_name
        mx = config.matrix
        self._store_path = Path(config.patchbay_home).expanduser() / mx.store_path
        ...
        from phoenix_patchbay.messenger.matrix.transport import MatrixTransport
        ...
        from phoenix_patchbay.messenger.matrix.message_queue import MatrixMessageQueue
    ...
    async def run(self) -> int:
        ...
        from phoenix_patchbay.messenger.matrix.startup import run_matrix_startup
    ...
    async def _on_media(self, room: MatrixRoom | object, event: RoomMessageMedia | object) -> None:
        ...
        from phoenix_patchbay.messenger.matrix.media import resolve_matrix_media
    ...
    async def _cmd_restart(
        ...
        from phoenix_patchbay.infra.restart import EXIT_RESTART, write_restart_marker

        marker = _expand_marker(self._config.patchbay_home)
    ...
    async def _cmd_showfiles(
        ...
        from phoenix_patchbay.messenger.matrix.file_browser import format_file_listing
    ...
    def _maybe_append_footer(self, result: object) -> None:
        ...
        if not self._config.scene.technical_footer or not result.model_name:
            return
        from phoenix_patchbay.text.response_format import format_technical_footer
    ...
    async def _send_selector_response(
        ...
        from phoenix_patchbay.messenger.matrix.buttons import REACTION_DIGITS
    ...
    async def _handle_button_callback(
        ...
        from phoenix_patchbay.messenger.callback_router import route_callback
    ...
    async def _handle_upgrade_callback(self, room_id: str, data: str) -> None:
        ...
        if data.startswith("upg:cl:"):
            ...
            if not version:
                return
            from phoenix_patchbay.infra.version import fetch_changelog
        ...
        if data.startswith("upg:yes:"):
            from phoenix_patchbay.infra.restart import EXIT_RESTART, write_restart_marker
            from phoenix_patchbay.infra.updater import perform_upgrade_pipeline
            from phoenix_patchbay.infra.version import get_current_version
    ...
    async def _handle_ns_callback(self, room_id: str, key: SessionKey, data: str) -> None:
        ...
        from phoenix_patchbay.messenger.telegram.callbacks import parse_ns_callback
