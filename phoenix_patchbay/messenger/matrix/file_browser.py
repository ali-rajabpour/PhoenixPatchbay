...
from phoenix_patchbay.files.browser import BROWSER_EXCLUDED_NAMES, list_directory
...
def format_file_listing(paths: PatchbayPaths, subdir: str = "") -> str:
    """Format a text listing of the workspace directory structure.

    Args:
        paths: Resolved workspace paths.
        subdir: Optional subdirectory relative to ``patchbay_home`` to list.
...
def _format_overview(paths: PatchbayPaths) -> str:
...
def _format_subdir(paths: PatchbayPaths, subdir: str) -> str:
    """Build a detailed listing for a specific subdirectory."""
    base = paths.patchbay_home.resolve()
    ...
    display_path = f"~/.phoenix-patchbay/{subdir}"
