...
from phoenix_patchbay.infra.restart import consume_restart_marker, consume_restart_sentinel

if TYPE_CHECKING:
    from phoenix_patchbay.messenger.matrix.bot import MatrixBot
...
async def _handle_restart_sentinel(bot: MatrixBot) -> dict[str, object] | None:
    ...
    if bot._orchestrator is None:
        return None
    sentinel_path = bot._orchestrator.paths.patchbay_home / "restart-sentinel.json"
...
def _consume_restart_marker(bot: MatrixBot) -> str:
    ...
    if paths_obj is None:
        return ""
    marker_path = paths_obj.patchbay_home / "restart-requested"
