...
from phoenix_patchbay.infra.restart import consume_restart_sentinel
from phoenix_patchbay.infra.updater import UpdateObserver, consume_upgrade_sentinel
...
async def _handle_restart_sentinel(bot: TelegramBot) -> dict[str, object] | None:
    ...
    sentinel_path = bot._orch.paths.patchbay_home / "restart-sentinel.json"
...
async def _handle_recovery(bot: TelegramBot, sentinel: dict[str, object] | None) -> None:
    ...
    upgrade = await asyncio.to_thread(consume_upgrade_sentinel, bot._orch.paths.patchbay_home)
    if upgrade:
    ...
    from phoenix_patchbay.infra.startup_state import detect_startup_kind, save_startup_state
    from phoenix_patchbay.text.response_format import startup_notification_text
    ...
    from phoenix_patchbay.infra.recovery import RecoveryPlanner
    from phoenix_patchbay.text.response_format import recovery_notification_text
...
async def _run_primary_startup(bot: TelegramBot) -> None:
    ...
    from phoenix_patchbay.infra.install import is_upgradeable
...
async def _ensure_managed_topics(bot: TelegramBot) -> None:
    ...
    from phoenix_patchbay.cleanup.consult_observer import ConsultWipeObserver
