...
from phoenix_patchbay.cli.timeout_controller import TimeoutConfig as TCConfig
...
from phoenix_patchbay.orchestrator.hooks import HookContext
from phoenix_patchbay.orchestrator.registry import OrchestratorResult
from phoenix_patchbay.session import SessionData, SessionKey
from phoenix_patchbay.text.response_format import session_error_text, timeout_error_text
from phoenix_patchbay.workspace.loader import build_appended_files_block, read_mainmemory
...
async def _gemini_missing_config_key_warning(
    ...
    """Warn when Gemini API-key mode is selected but Patchbay config key is empty."""
    ...
    # startup. This avoids forcing a patchbay restart for a simple auth flip.
...
async def normal(
    ...
    *,
    ...
    if warning is not None:
        logger.warning("Gemini API-key mode without configured patchbay key")
        return warning
