...
from phoenix_patchbay.workspace.init import inject_runtime_environment
...
def _docker_skill_resync(paths: PatchbayPaths) -> None:
...
async def create_orchestrator(
    ...
    *,
    ...
    if agent_name == "main":
        os.environ["PATCHBAY_HOME"] = str(paths.patchbay_home)
...
async def start_api_server(
    ...
    try:
        ...
        logger.warning(
            "API server enabled but PyNaCl is not installed. Install with: pip install patchbay[api]"
        )
        return
...
async def shutdown(orch: Orchestrator) -> None:
    ...
    await asyncio.to_thread(cleanup_patchbay_links, orch._paths)
