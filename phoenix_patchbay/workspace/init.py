...
from phoenix_patchbay.workspace.cron_tasks import ensure_task_rule_files
...
from phoenix_patchbay.workspace.skill_sync import sync_bundled_skills, sync_skills
...
_ZONE2_PY_DIRS = frozenset(
    {
        ...
        # (PATCHBAY_TRANSCRIBE_COMMAND, PATCHBAY_VIDEO_TRANSCRIBE_COMMAND) — keep
...
def _sync_home_defaults(paths: PatchbayPaths) -> None:
    """Walk the home-defaults template and copy to ``patchbay_home``.

    The template at ``<repo>/workspace/`` mirrors ``~/.phoenix-patchbay/`` exactly.
    Zone rules per file:
    ...
    if not paths.home_defaults.is_dir():
        ...
        return
    _walk_and_copy(paths.home_defaults, paths.patchbay_home)
    ...
    if paths.patchbay_home.parent.name != "agents":
...
def _smart_merge_config(paths: PatchbayPaths) -> None:
...
def _migrate_tasks_to_cron_tasks(paths: PatchbayPaths) -> None:
...
def _clean_orphan_symlinks(paths: PatchbayPaths) -> None:
...
def _ensure_required_dirs(paths: PatchbayPaths) -> None:
    """Create any required directories that are missing."""
    for rel in _REQUIRED_DIRS:
        d = paths.patchbay_home / rel
        if not d.is_dir():
            d.mkdir(parents=True, exist_ok=True)
            logger.info("Created missing directory: %s", d)


def init_workspace(paths: PatchbayPaths) -> None:
    """Initialize the workspace: defaults sync, rule sync, config merge, cleanup."""
    logger.info("Workspace init started home=%s", paths.patchbay_home)
...
- Your filesystem is isolated. `/patchbay` is the mounted host directory `~/.phoenix-patchbay`.
