...
class RulesSelector:
    ...
    Deployed naming in ~/.phoenix-patchbay/:
    ...
    def deploy_rules(self) -> None:
        """Auto-discover and deploy all rule files to ~/.phoenix-patchbay/.

        Scans _home_defaults/ for directories with RULES templates, selects
        the best variant for current auth state, and deploys to ~/.phoenix-patchbay/
        ...
        for template_dir in template_dirs:
            ...
            dst_dir = self._paths.patchbay_home / rel_path
    ...
    def _remove_files_by_name(self, filename: str) -> int:
        """Remove all files with given name in ~/.phoenix-patchbay/.
        ...
        """
        cron_tasks_path = self._paths.patchbay_home / "workspace" / "cron_tasks"
        removed_count = 0
        for file_path in self._paths.patchbay_home.rglob(filename):
            ...
            try:
                ...
                removed_count += 1
                logger.debug(
                    "Removed stale file: %s", file_path.relative_to(self._paths.patchbay_home)
                )
            except OSError:
