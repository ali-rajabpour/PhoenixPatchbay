"""Cross-platform skill directory sync between patchbay workspace and CLI tools.

Provides multi-way symlink synchronization so skills installed via Claude Code,
Codex CLI, Gemini CLI, or the patchbay workspace are visible to all agents.

Includes bundled-skill linking (package → workspace), sync-time external-symlink
protection, and cleanup of patchbay-created links on shutdown.

When Docker sandboxing is active, symlinks are replaced with directory copies
(marked with ``.phoenix-patchbay_managed``) because absolute host paths do not resolve
...
_MANAGED_MARKER = ".phoenix-patchbay_managed"
...
def _resolve_canonical(
    ...
    *registries: dict[str, Path],
    ...
    Priority follows argument order (typically patchbay > claude > codex > gemini).
...
def _is_managed_copy(path: Path) -> bool:
    """Return ``True`` if *path* is a patchbay-managed copy (has marker file)."""
...
def _ensure_copy(dest: Path, source: Path) -> bool:
    """Copy *source* directory to *dest* with a ``.phoenix-patchbay_managed`` marker.
...
def sync_skills(paths: PatchbayPaths, *, docker_active: bool = False) -> None:
    """Multi-way skill directory sync: patchbay workspace <-> CLI skill dirs.

    Syncs between patchbay workspace, ~/.claude/skills, ~/.codex/skills,
    and ~/.gemini/skills.
    ...
    all_dirs: dict[str, Path] = {"patchbay": paths.skills_dir, **cli_dirs}
    ...
    # Priority order: patchbay > claude > codex > gemini
    priority = ("patchbay", "claude", "codex", "gemini", "grok")
...
def _clean_invalid_workspace_skill_links(base_dir: Path) -> int:
    """Remove Patchbay workspace links/copies that point at invalid skill folders.
...
def _iter_bundled_entries(paths: PatchbayPaths) -> list[tuple[Path, Path]]:
...
def sync_bundled_skills(paths: PatchbayPaths, *, docker_active: bool = False) -> None:
    """Sync bundled skills from the package into the patchbay workspace.

    Creates symlinks (or copies when *docker_active*) from
    ``~/.phoenix-patchbay/workspace/skills/<name>`` to the package's
    ``_home_defaults/workspace/skills/<name>`` so bundled skills
    stay up-to-date with the installed patchbay version.
...
def cleanup_patchbay_links(paths: PatchbayPaths) -> int:
    """Remove symlinks created by patchbay in CLI skill directories.

    Only removes symlinks whose resolved target is under the patchbay workspace
    ...
    for cli_dir in _cli_skill_dirs().values():
        ...
        for entry in cli_dir.iterdir():
            ...
            if any(_is_under(resolved, root) for root in managed_roots):
                entry.unlink()
                removed += 1
                logger.info("Removed patchbay skill link: %s", entry)

    if removed:
        logger.info("Cleaned up %d patchbay skill link(s) from CLI directories", removed)
    return removed
