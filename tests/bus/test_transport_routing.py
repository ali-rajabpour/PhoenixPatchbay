...
from phoenix_patchbay.bus.envelope import DeliveryMode, Envelope, Origin
...
class TestCascadingFallback:
    ...
    async def test_fallback_logs_warning(self, caplog: pytest.LogCaptureFixture) -> None:
        ...
        env = _env(transport="mx", delivery=DeliveryMode.UNICAST)
        with caplog.at_level("WARNING", logger="phoenix_patchbay.bus.bus"):
            await bus.submit(env)
    ...
    async def test_fallback_delivery_failure_is_logged(
        ...
        env = _env(transport="mx", delivery=DeliveryMode.UNICAST)
        with caplog.at_level("ERROR", logger="phoenix_patchbay.bus.bus"):
            await bus.submit(env)
...
class TestAdapterTransportParam:
    def test_from_cron_result_includes_transport(self) -> None:
        ...
        from phoenix_patchbay.bus.adapters import from_cron_result
    ...
    def test_from_heartbeat_includes_transport(self) -> None:
        ...
        from phoenix_patchbay.bus.adapters import from_heartbeat
...
class TestCronJobTransport:
    def test_cron_job_has_transport_field(self) -> None:
        ...
        from phoenix_patchbay.cron.manager import CronJob
...
# -- Executor PATCHBAY_TRANSPORT env var --


class TestExecutorTransportEnv:
    def testbuild_subprocess_env_includes_transport(self) -> None:
        """build_subprocess_env sets PATCHBAY_TRANSPORT from CLIConfig."""
        ...
        assert env["PATCHBAY_TRANSPORT"] == "mx"

    def testbuild_subprocess_env_default_transport(self) -> None:
        """build_subprocess_env defaults PATCHBAY_TRANSPORT to 'tg'."""
        from phoenix_patchbay.cli.base import CLIConfig
        from phoenix_patchbay.cli.executor import build_subprocess_env
