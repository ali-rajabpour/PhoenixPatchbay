...
from phoenix_patchbay.cleanup.observer import CleanupObserver, _delete_old_files
from phoenix_patchbay.config import AgentConfig, CleanupConfig
...
def _make_paths(tmp_path: Path) -> PatchbayPaths:
    return PatchbayPaths(patchbay_home=tmp_path)
...
async def test_maybe_run_skips_wrong_hour(tmp_path: Path) -> None:
    ...
    with patch("phoenix_patchbay.cleanup.observer.datetime") as mock_dt:
