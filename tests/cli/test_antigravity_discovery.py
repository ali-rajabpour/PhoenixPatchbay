...
from phoenix_patchbay.cli.antigravity_cache import (
    _FALLBACK_ANTIGRAVITY_MODELS,
    AntigravityModelCache,
)
from phoenix_patchbay.cli.antigravity_discovery import _parse_models, discover_antigravity_models
from phoenix_patchbay.cli.antigravity_runtime import (
...
async def test_discover_returns_models_on_success() -> None:
    with (
        patch(
            "phoenix_patchbay.cli.antigravity_discovery.asyncio.create_subprocess_exec",
            return_value=_mock_proc(_SAMPLE_OUTPUT.encode()),
        ) as create_process,
        patch(
            "phoenix_patchbay.cli.antigravity_discovery.antigravity_process_env",
...
async def test_cache_falls_back_when_discovery_empty(tmp_path: Path) -> None:
    ...
    with patch(
        "phoenix_patchbay.cli.antigravity_cache.discover_antigravity_models",
...
async def test_cache_persists_models_from_official_settings(tmp_path: Path) -> None:
    ...
    with (
        ...
        patch(
            "phoenix_patchbay.cli.antigravity_cache.configured_antigravity_models",
