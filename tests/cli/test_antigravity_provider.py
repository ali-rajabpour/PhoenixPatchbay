...
from phoenix_patchbay.cli.antigravity_provider import (
    AntigravityCLI,
    _agy_state_root,
    _read_transcript_answer,
)
from phoenix_patchbay.cli.base import CLIConfig
from phoenix_patchbay.cli.stream_events import AssistantTextDelta, ResultEvent
...
def test_antigravity_dotted_workspace_mapped_to_symlink(tmp_path: Path) -> None:
    # agy rejects dot-prefixed ancestors (e.g. ~/.phoenix-patchbay/workspace), so patchbay
    # exposes the workspace via a non-dotted symlink that still resolves back.
    dotted = tmp_path / ".phoenix-patchbay" / "workspace"
...
def test_antigravity_ignores_docker_container() -> None:
    cli = AntigravityCLI(
        CLIConfig(
            ...
            docker_container="patchbay-sandbox",
            working_dir=".",
        )
    )
...
class TestStreaming:
    ...
    async def test_send_streaming_emits_text_then_result(self) -> None:
        ...
        with patch(
            "phoenix_patchbay.cli.antigravity_provider.asyncio.create_subprocess_exec",
            return_value=proc,
        ):
...
class TestAgentEnvInjection:
    """agy subprocesses must receive the PATCHBAY_* agent identification env."""
    ...
    async def test_send_injects_agent_env(self) -> None:
        ...
        assert env["PATCHBAY_AGENT_NAME"] == "main"
        assert env["PATCHBAY_CHAT_ID"] == "77"
        assert env["PATCHBAY_TRANSPORT"] == "tg"
        assert "PATCHBAY_HOME" in env
        assert "PATCHBAY_SHARED_MEMORY_PATH" in env
...
def isolated_agy_state(tmp_path_factory: pytest.TempPathFactory) -> Iterator[Path]:
    ...
    with patch("phoenix_patchbay.cli.antigravity_provider._agy_state_root", return_value=root):
        yield root
