...
def _patch_claude_cli_fallback(monkeypatch: pytest.MonkeyPatch, *, logged_in: bool = False) -> None:
    ...
    import phoenix_patchbay.cli.auth as _auth_mod
...
def test_check_gemini_auth_patchbay_config_key(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    import phoenix_patchbay.cli.auth as _auth_mod
    ...
    patchbay_config = tmp_path / ".phoenix-patchbay" / "config" / "config.json"
    patchbay_config.parent.mkdir(parents=True)
    patchbay_config.write_text('{"gemini_api_key":"from-patchbay-config"}')

    result = check_gemini_auth()

    assert result.status == AuthStatus.AUTHENTICATED
    assert result.auth_file == patchbay_config


def test_check_gemini_auth_patchbay_config_null_string_ignored(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    import phoenix_patchbay.cli.auth as _auth_mod
    ...
    patchbay_config = tmp_path / ".phoenix-patchbay" / "config" / "config.json"
    patchbay_config.parent.mkdir(parents=True)
    patchbay_config.write_text('{"gemini_api_key":"null"}')
