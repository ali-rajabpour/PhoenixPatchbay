...
from phoenix_patchbay.cli.base import CLIConfig, add_cli_opt
from phoenix_patchbay.cli.claude_provider import (
...
_EXEC_PATH = "phoenix_patchbay.cli.executor.asyncio.create_subprocess_exec"
...
def _default_non_windows_cli(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("phoenix_patchbay.cli.base._IS_WINDOWS", False)
    monkeypatch.setattr("phoenix_patchbay.cli.executor._IS_WINDOWS", False)
    monkeypatch.setattr("phoenix_patchbay.cli.claude_provider._IS_WINDOWS", False)
...
def _make_cli(
    ...
    **kwargs: Any,
    ...
    monkeypatch.setattr("phoenix_patchbay.cli.claude_provider.which", lambda _: "/usr/bin/claude")
    cfg = CLIConfig(
        provider="claude",
        model=model,
...
class TestInit:
    def test_find_cli_not_found_raises(self, monkeypatch: pytest.MonkeyPatch) -> None:
        monkeypatch.setattr("phoenix_patchbay.cli.claude_provider.which", lambda _: None)
...
class TestSend:
    ...
    async def test_windows_docker_keeps_stdin_open(self, monkeypatch: pytest.MonkeyPatch) -> None:
        monkeypatch.setattr("phoenix_patchbay.cli.claude_provider._IS_WINDOWS", True)
        ...
        with (
            patch("phoenix_patchbay.cli.claude_provider.docker_wrap", return_value=wrapped) as wrap,
            patch(
                "phoenix_patchbay.cli.claude_provider.run_oneshot_subprocess",
...
class TestSendStreaming:
    ...
    async def test_windows_docker_streaming_keeps_stdin_open(
        ...
        with (
            patch("phoenix_patchbay.cli.claude_provider.docker_wrap", return_value=wrapped) as wrap,
            patch("phoenix_patchbay.cli.claude_provider.run_streaming_subprocess", _empty_stream),
        ):
            await _collect_stream(cli)
...
class TestLogCmd:
    def test_truncates_long_values_after_flags(self, caplog: pytest.LogCaptureFixture) -> None:
        ...
        with caplog.at_level(logging.INFO, logger="phoenix_patchbay.cli.claude_provider"):
            _log_cmd(cmd)
...
class TestAppendSystemPromptFile:
    ...
    async def test_small_append_stays_inline(self, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        with (
            ...
            patch("phoenix_patchbay.cli.claude_provider.create_system_prompt_file") as make_file,
        ):
            await cli.send("hi")
    ...
    async def test_docker_uses_container_path(
        ...
        fake_home = tmp_path / ".phoenix-patchbay"
        fake_home.mkdir()
        monkeypatch.setattr(
            "phoenix_patchbay.workspace.paths.resolve_paths",
            lambda: SimpleNamespace(patchbay_home=fake_home),
        )
        ...
        def _exec(*args: Any, **_kwargs: Any) -> MagicMock:
            ...
            host_path = str(fake_home) + arg_path[len("/patchbay") :]
        ...
        assert captured["arg_path"].startswith("/patchbay/tmp/")
