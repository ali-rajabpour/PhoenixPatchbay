...
from phoenix_patchbay.cli.codex_provider import (
    CodexCLI,
    _codex_final_result,
    _log_cmd,
    _StreamState,
)
from phoenix_patchbay.cli.executor import SubprocessResult
...
def _make_cli(monkeypatch: pytest.MonkeyPatch, **overrides: Any) -> CodexCLI:
    monkeypatch.setattr("phoenix_patchbay.cli.codex_provider.which", lambda _: "/usr/bin/codex")
    return CodexCLI(
        CLIConfig(
            provider="codex",
...
class TestInit:
    def test_find_cli_raises_when_not_on_path(self, monkeypatch: pytest.MonkeyPatch) -> None:
        monkeypatch.setattr("phoenix_patchbay.cli.codex_provider.which", lambda _: None)
    ...
    def test_find_cli_uses_resolved_path(self, monkeypatch: pytest.MonkeyPatch) -> None:
        monkeypatch.setattr("phoenix_patchbay.cli.codex_provider.which", lambda _: "/opt/bin/codex")
...
class TestSend:
    async def test_send_success(self, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        with patch("phoenix_patchbay.cli.executor.asyncio") as mock_asyncio:
...
class TestLogCmd:
    def test_short_values_not_truncated(self, caplog: pytest.LogCaptureFixture) -> None:
        with caplog.at_level(logging.INFO, logger="phoenix_patchbay.cli.codex_provider"):
...
class TestEdgeCases:
    ...
    def test_parse_output_error_logged(self, caplog: pytest.LogCaptureFixture) -> None:
        with caplog.at_level(logging.ERROR, logger="phoenix_patchbay.cli.codex_provider"):
