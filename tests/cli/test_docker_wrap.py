...
from phoenix_patchbay.cli.base import CLIConfig, docker_wrap
...
def test_docker_wrap_with_container() -> None:
    ...
    assert result_cmd == [
        ...
        "PATCHBAY_CHAT_ID=42",
        "-e",
        "PATCHBAY_TRANSPORT=tg",
        "-e",
        "PATCHBAY_AGENT_NAME=main",
        "-e",
        "PATCHBAY_INTERAGENT_PORT=8799",
        "-e",
        "PATCHBAY_HOME=/patchbay",
        "-e",
        "PATCHBAY_SHARED_MEMORY_PATH=/patchbay/SHAREDMEMORY.md",
        "-e",
...
def test_docker_wrap_injects_chat_id() -> None:
    ...
    result_cmd, _ = docker_wrap(cmd, cfg)
    assert "PATCHBAY_CHAT_ID=999" in result_cmd
...
def test_docker_wrap_sub_agent_container_paths() -> None:
    """Sub-agent working_dir maps to /patchbay/agents/<name>/workspace inside container."""
    cmd = ["claude", "-p", "hi"]
    cfg = CLIConfig(
        docker_container="sandbox",
        chat_id=1,
        working_dir="/home/user/.phoenix-patchbay/agents/test/workspace",
        agent_name="test",
    )
    result_cmd, cwd = docker_wrap(cmd, cfg)
    assert cwd is None
    ...
    assert result_cmd[w_idx + 1] == "/patchbay/agents/test/workspace"
    # PATCHBAY_HOME is the sub-agent home inside the container
    assert "PATCHBAY_HOME=/patchbay/agents/test" in result_cmd
    # Shared memory is at the root
    assert "PATCHBAY_SHARED_MEMORY_PATH=/patchbay/SHAREDMEMORY.md" in result_cmd


def test_docker_wrap_main_agent_container_paths() -> None:
    """Main agent working_dir maps to /patchbay/workspace inside container."""
    cmd = ["claude", "-p", "hi"]
    cfg = CLIConfig(
        docker_container="sandbox",
        chat_id=1,
        working_dir="/home/user/.phoenix-patchbay/workspace",
        agent_name="main",
    )
    result_cmd, _ = docker_wrap(cmd, cfg)
    w_idx = result_cmd.index("-w")
    assert result_cmd[w_idx + 1] == "/patchbay/workspace"
    assert "PATCHBAY_HOME=/patchbay" in result_cmd
    assert "PATCHBAY_SHARED_MEMORY_PATH=/patchbay/SHAREDMEMORY.md" in result_cmd
...
def test_docker_wrap_sub_agent_windows_paths_are_posix() -> None:
    ...
    cfg = CLIConfig(
        docker_container="sandbox",
        chat_id=1,
        working_dir=PureWindowsPath(r"C:\Users\me\.phoenix-patchbay\agents\seismic-bot\workspace"),
        agent_name="seismic-bot",
    )
    result_cmd, _ = docker_wrap(cmd, cfg, interactive=True)
    w_idx = result_cmd.index("-w")
    assert result_cmd[w_idx + 1] == "/patchbay/agents/seismic-bot/workspace"
    assert "PATCHBAY_HOME=/patchbay/agents/seismic-bot" in result_cmd
