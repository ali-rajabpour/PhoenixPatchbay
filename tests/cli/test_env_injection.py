...
def test_subprocess_env_works_without_env_file(tmp_path: Path) -> None:
    ...
    assert env is not None
    assert "PATCHBAY_AGENT_NAME" in env


def test_subprocess_env_sets_task_id_for_task_label(tmp_path: Path) -> None:
    """Background-task labels (task:<id>) expose PATCHBAY_TASK_ID to the subprocess."""
    workspace = tmp_path / "workspace"
    workspace.mkdir()
    ...
    assert env is not None
    assert env["PATCHBAY_TASK_ID"] == "abc123"


def test_subprocess_env_omits_task_id_for_other_labels(tmp_path: Path) -> None:
    """Non-task labels (main, ns:*, interagent:*) must not leak PATCHBAY_TASK_ID."""
    workspace = tmp_path / "workspace"
    workspace.mkdir()
    ...
    for label in ("main", "ns:build", "interagent:worker"):
        ...
        assert env is not None
        assert "PATCHBAY_TASK_ID" not in env


def test_docker_wrap_sets_task_id_for_task_label(tmp_path: Path) -> None:
    """Docker exec gets PATCHBAY_TASK_ID as an -e flag for task labels."""
    workspace = tmp_path / "workspace"
    workspace.mkdir()
    ...
    assert "PATCHBAY_TASK_ID=abc123" in cmd
