...
if TYPE_CHECKING:
    import pytest
from phoenix_patchbay.cli.base import host_path_to_container
from phoenix_patchbay.cli.gemini_provider import GeminiCLI, _log_cmd, _parse_response
...
def _make_cli(monkeypatch: pytest.MonkeyPatch, **overrides: Any) -> GeminiCLI:
    monkeypatch.setattr("phoenix_patchbay.cli.gemini_provider.find_gemini_cli", lambda: "/usr/bin/gemini")
    monkeypatch.setattr("phoenix_patchbay.cli.gemini_provider.find_gemini_cli_js", lambda: None)
    return GeminiCLI(
        CLIConfig(
            provider="gemini",
...
class TestPrepareEnv:
    def test_prepends_cli_parent_when_cli_path_is_absolute(
        ...
        monkeypatch.setattr(
            "phoenix_patchbay.cli.gemini_provider.find_gemini_cli",
            lambda: "/opt/node/v22.0.0/bin/gemini",
        )
        monkeypatch.setattr(
            "phoenix_patchbay.cli.gemini_provider.find_gemini_cli_js",
    ...
    def test_host_to_container_path_normalizes_windows_separators(
        ...
        fake_paths = type("P", (), {"patchbay_home": Path(r"C:\Users\ZOZN109\.phoenix-patchbay")})()
        monkeypatch.setattr(
            "phoenix_patchbay.workspace.paths.resolve_paths",
            lambda: fake_paths,
        )

        result = host_path_to_container(r"C:\Users\ZOZN109\.phoenix-patchbay\tmp\gemini_system_abc.md")

        assert result == "/patchbay/tmp/gemini_system_abc.md"
...
class TestSend:
    async def test_send_success(self, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        with patch(
            "phoenix_patchbay.cli.gemini_provider.asyncio.create_subprocess_exec", return_value=proc
        ):
