...
from phoenix_patchbay.cli.grok_cache import _FALLBACK_GROK_MODELS, GrokModelCache
from phoenix_patchbay.cli.grok_discovery import _parse_models, discover_grok_models
...
async def test_discover_returns_models_on_success() -> None:
    with (
        patch("phoenix_patchbay.cli.grok_discovery.which", return_value="/usr/bin/grok"),
        patch(
            "phoenix_patchbay.cli.grok_discovery.asyncio.create_subprocess_exec",
...
async def test_discover_returns_empty_when_binary_missing() -> None:
    with patch("phoenix_patchbay.cli.grok_discovery.which", return_value=None):
        assert await discover_grok_models() == ()
...
async def test_cache_persists_discovered_models(tmp_path: Path) -> None:
    ...
    with patch(
        "phoenix_patchbay.cli.grok_cache.discover_grok_models",
