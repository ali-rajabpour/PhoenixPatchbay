...
from phoenix_patchbay.cli.grok_provider import GrokCLI, _parse_response
...
from phoenix_patchbay.cron.execution import _build_grok_cmd
...
class TestGrokProvider:
    def test_find_cli_missing(self, monkeypatch: pytest.MonkeyPatch) -> None:
        monkeypatch.setattr("phoenix_patchbay.cli.grok_provider.which", lambda _: None)
    ...
    def test_build_command_flags(self, monkeypatch: pytest.MonkeyPatch) -> None:
        monkeypatch.setattr("phoenix_patchbay.cli.grok_provider.which", lambda _: "/usr/bin/grok")
        cli = GrokCLI(
            CLIConfig(
                provider="grok",
                model="grok-4.5",
    ...
    def test_long_prompt_uses_file(self, monkeypatch: pytest.MonkeyPatch) -> None:
        monkeypatch.setattr("phoenix_patchbay.cli.grok_provider.which", lambda _: "/usr/bin/grok")
        monkeypatch.setattr("phoenix_patchbay.cli.grok_provider._IS_WINDOWS", False)
...
class TestGrokCronCmd:
    def test_short_prompt_uses_p(self, monkeypatch: pytest.MonkeyPatch) -> None:
        monkeypatch.setattr("phoenix_patchbay.cron.execution.which", lambda _: "/usr/bin/grok")
