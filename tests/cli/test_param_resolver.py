...
from phoenix_patchbay.cli.param_resolver import (
    RunOverrides,
    resolve_cli_config,
)
...
def test_resolve_global_only(base_config: AgentConfig, codex_cache: CodexModelCache) -> None:
    ...
    assert result.working_dir == "~/patchbay"
...
def test_resolve_invalid_claude_model(
    ...
    with pytest.raises(PatchbayError, match="Invalid Claude model"):
...
def test_resolve_invalid_codex_model(
    ...
    with pytest.raises(PatchbayError, match="Invalid Codex model"):
        resolve_cli_config(base_config, codex_cache, task_overrides=overrides)
...
def test_resolve_rejects_unsupported_task_provider(
    ...
    with pytest.raises(PatchbayError, match="Unsupported task provider: antigravity"):
        resolve_cli_config(base_config, codex_cache, task_overrides=overrides)
...
def test_resolve_codex_effort_fallback(
    ...
    with pytest.raises(PatchbayError, match="Invalid reasoning effort"):
        resolve_cli_config(base_config, codex_cache, task_overrides=overrides)
...
def test_resolve_claude_rejects_invalid_reasoning(
    ...
    with pytest.raises(PatchbayError):
        resolve_cli_config(
...
def test_resolve_gemini_invalid_against_discovered_models(
    ...
    with pytest.raises(PatchbayError, match="Invalid Gemini model"):
        resolve_cli_config(base_config, codex_cache, task_overrides=overrides)
