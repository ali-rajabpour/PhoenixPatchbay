...
from phoenix_patchbay.cli.stream_events import StreamEvent, ToolUseEvent
from phoenix_patchbay.cli.types import AgentRequest, CLIResponse
...
async def test_execute_returns_agent_response() -> None:
    ...
    with patch("phoenix_patchbay.cli.service.create_cli") as mock_create:
        mock_cli = AsyncMock()
...
async def test_execute_streaming_success() -> None:
    ...
    from phoenix_patchbay.cli.stream_events import AssistantTextDelta, ResultEvent, ThinkingEvent
...
async def test_stream_callbacks_dispatches_compact_boundary() -> None:
    ...
    from phoenix_patchbay.cli.service import _StreamCallbacks
    from phoenix_patchbay.cli.stream_events import CompactBoundaryEvent
...
async def test_stream_callbacks_dispatches_thinking_text() -> None:
    from phoenix_patchbay.cli.service import _StreamCallbacks
    from phoenix_patchbay.cli.stream_events import ThinkingEvent
...
def test_make_cli_docker_mode_ignores_resolver() -> None:
    svc = _make_service(working_dir="/default/workspace", docker_container="patchbay-sandbox")
...
def test_docker_enabled_property() -> None:
    ...
    assert _make_service(docker_container="patchbay-sandbox").docker_enabled is True
