...
from phoenix_patchbay.cli.codex_discovery import CodexModelInfo
from phoenix_patchbay.config import AgentConfig
from phoenix_patchbay.cron.execution import (
    enrich_instruction,
    parse_claude_result,
    parse_codex_result,
)
...
def _make_manager(paths: PatchbayPaths) -> CronManager:
...
def _write_jobs(paths: PatchbayPaths, jobs: list[CronJob]) -> None:
...
class TestCronObserverScheduling:
    ...
    async def test_request_reschedule_coalesces_pending_task(self, tmp_path: Path) -> None:
        ...
        with patch(
            "phoenix_patchbay.cron.observer.asyncio.create_task", side_effect=_create_task
        ) as create:
...
class TestCronObserverExecution:
    ...
    async def test_preflight_skip_avoids_agent_execution(self, tmp_path: Path) -> None:
        ...
        with (
            ...
            patch("phoenix_patchbay.cron.observer.execute_in_task_folder") as execute,
        ):
    ...
    def test_kill_process_group_kills_group_for_real_pid(self) -> None:
        from phoenix_patchbay.cron.observer import _kill_process_group

        proc = MagicMock()
        proc.pid = 1234
        with (
            patch("phoenix_patchbay.cron.observer.os.getpgid", return_value=1234) as getpgid,
            patch("phoenix_patchbay.cron.observer.os.killpg") as killpg,
        ):
    ...
    def test_kill_process_group_never_signals_pgid_one(self) -> None:
        ...
        proc.pid = 1234
        with (
            patch("phoenix_patchbay.cron.observer.os.getpgid", return_value=1),
            patch("phoenix_patchbay.cron.observer.os.killpg") as killpg,
        ):
            _kill_process_group(proc)
    ...
    def test_kill_process_group_rejects_non_int_pid(self) -> None:
        ...
        with (
            patch("phoenix_patchbay.cron.observer.os.getpgid") as getpgid,
            patch("phoenix_patchbay.cron.observer.os.killpg") as killpg,
        ):
            _kill_process_group(proc)
    ...
    def test_kill_process_group_falls_back_when_group_gone(self) -> None:
        ...
        proc.pid = 1234
        with (
            patch("phoenix_patchbay.cron.observer.os.getpgid", side_effect=ProcessLookupError),
            patch("phoenix_patchbay.cron.observer.os.killpg") as killpg,
        ):
            _kill_process_group(proc)
    ...
    async def test_handles_missing_cli(self, tmp_path: Path) -> None:
        ...
        with (
            ...
            patch("phoenix_patchbay.cron.execution.which", return_value=None),
        ):
    ...
    async def test_executes_claude_job_success(self, tmp_path: Path) -> None:
        ...
        with (
            time_machine.travel(datetime(2026, 1, 15, 14, 0, tzinfo=UTC)),
            patch("phoenix_patchbay.cron.execution.which", return_value="/usr/bin/claude"),
    ...
    async def test_executes_codex_job_success(self, tmp_path: Path) -> None:
        ...
        with (
            time_machine.travel(datetime(2026, 1, 15, 14, 0, tzinfo=UTC)),
            patch("phoenix_patchbay.cron.execution.which", return_value="/usr/bin/codex"),
            patch("asyncio.create_subprocess_exec", return_value=mock_proc) as exec_mock,
        ):
            await observer._execute_job("daily", "Generate report", "daily")
    ...
    async def test_retries_preserved_result_without_rerunning_agent(self, tmp_path: Path) -> None:
        ...
        with patch("phoenix_patchbay.cron.observer.execute_in_task_folder") as execute:
...
class TestCronResultDelivery:
    ...
    async def test_run_at_catches_unexpected_exception(self, tmp_path: Path) -> None:
        ...
        from phoenix_patchbay.cron.observer import _ScheduledJob
