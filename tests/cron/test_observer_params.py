...
from phoenix_patchbay.cron.execution import OneShotCommand
...
class TestResolveExecutionConfig:
    ...
    def test_resolve_execution_config_no_overrides(
        ...
        with patch("phoenix_patchbay.infra.base_oneshot_observer.resolve_cli_config") as mock_resolve:
            mock_resolve.return_value = CLIRunConfig(
                provider="claude",
                model="opus",
                reasoning_effort="",
                cli_parameters=[],
...
class TestExecuteJobWithOverrides:
    ...
    async def test_execute_job_with_model_override(
        ...
        with (
            ...
            patch("phoenix_patchbay.cron.execution.build_cmd") as mock_build,
            patch("phoenix_patchbay.infra.base_oneshot_observer.resolve_cli_config") as mock_resolve,
    ...
    async def test_execute_job_with_cli_parameters(
        ...
        # Verify build_cmd was called with CLIRunConfig containing parameters
        mock_build.assert_called_once()
        exec_config = mock_build.call_args[0][0]
        assert isinstance(exec_config, CLIRunConfig)
