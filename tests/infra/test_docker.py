...
from phoenix_patchbay.config import DockerConfig
from phoenix_patchbay.workspace.paths import PatchbayPaths


@pytest.fixture
def docker_paths(tmp_path: Path) -> PatchbayPaths:
...
class TestDockerManager:
    ...
    async def test_setup_returns_none_when_docker_not_found(
        self, docker_config: DockerConfig, docker_paths: PatchbayPaths
    ) -> None:
    ...
    async def test_mounts_full_patchbay_home(
        self, docker_config: DockerConfig, docker_paths: PatchbayPaths
    ) -> None:
        """Verify run command mounts entire ~/.phoenix-patchbay, not just workspace."""
        from phoenix_patchbay.infra.docker import DockerManager
        ...
        # Full patchbay_home mounted at /patchbay
        assert f"{docker_paths.patchbay_home}:/patchbay" in run_str
        # Working dir is /patchbay/workspace
        assert "-w /patchbay/workspace" in run_str
        # PATCHBAY_HOME env var set inside container
        assert "PATCHBAY_HOME=/patchbay" in run_str
    ...
    async def test_sub_agent_mounts_root_patchbay_home(
        self, docker_config: DockerConfig, tmp_path: Path
    ) -> None:
        """Sub-agent container mounts ~/.phoenix-patchbay (root), not ~/.phoenix-patchbay/agents/test."""
        from phoenix_patchbay.infra.docker import DockerManager

        root_home = tmp_path / ".phoenix-patchbay"
        ...
        fw.mkdir()
        paths = PatchbayPaths(
            patchbay_home=agent_home, home_defaults=fw / "workspace", framework_root=fw
        )
        ...
        assert f"{root_home}:/patchbay" in run_str
        assert f"{agent_home}:/patchbay" not in run_str
