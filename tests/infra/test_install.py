...
from phoenix_patchbay.infra.install import detect_install_mode, is_upgradeable
...
class TestDetectInstallMode:
    ...
    def test_pipx_detected_from_sys_prefix(self) -> None:
        with patch("phoenix_patchbay.infra.install.sys") as mock_sys:
            mock_sys.prefix = "/home/user/.local/share/pipx/venvs/patchbay"
    ...
    def test_editable_install_detected_as_dev(self) -> None:
        ...
        with (
            patch("phoenix_patchbay.infra.install.sys") as mock_sys,
            patch("phoenix_patchbay.infra.install.distribution", return_value=mock_dist),
        ):
    ...
    def test_package_not_found_is_dev(self) -> None:
        ...
        with (
            patch("phoenix_patchbay.infra.install.sys") as mock_sys,
            patch(
                "phoenix_patchbay.infra.install.distribution",
                side_effect=PackageNotFoundError("patchbay"),
            ),
        ):
            mock_sys.prefix = "/usr"
            assert detect_install_mode() == "dev"
    ...
    def test_metadata_error_falls_back_to_dev(self) -> None:
        with (
            patch("phoenix_patchbay.infra.install.sys") as mock_sys,
            patch("phoenix_patchbay.infra.install.distribution", side_effect=OSError("corrupt")),
        ):
            mock_sys.prefix = "/usr"
            assert detect_install_mode() == "dev"
    ...
    def test_pipx_path_variant_windows(self) -> None:
        with patch("phoenix_patchbay.infra.install.sys") as mock_sys:
            mock_sys.prefix = "C:\\Users\\me\\AppData\\Local\\pipx\\venvs\\patchbay"
            assert detect_install_mode() == "pipx"

    def test_uv_tool_detected_from_sys_prefix(self) -> None:
        with patch("phoenix_patchbay.infra.install.sys") as mock_sys:
            mock_sys.prefix = "/home/user/.local/share/uv/tools/patchbay"
            mock_sys.executable = "/home/user/.local/share/uv/tools/patchbay/bin/python"
...
class TestIsUpgradeable:
    ...
    def test_pipx_is_upgradeable(self) -> None:
        with patch("phoenix_patchbay.infra.install.detect_install_mode", return_value="pipx"):
            assert is_upgradeable() is True

    def test_pip_is_upgradeable(self) -> None:
        with patch("phoenix_patchbay.infra.install.detect_install_mode", return_value="pip"):
            assert is_upgradeable() is True

    def test_uv_is_upgradeable(self) -> None:
        with patch("phoenix_patchbay.infra.install.detect_install_mode", return_value="uv"):
            assert is_upgradeable() is True

    def test_dev_is_not_upgradeable(self) -> None:
        with patch("phoenix_patchbay.infra.install.detect_install_mode", return_value="dev"):
