...
from phoenix_patchbay.infra.service_linux import (
    _SERVICE_NAME,
    _enable_linger,
...
class TestGenerateServiceUnit:
    def test_contains_binary_path(self) -> None:
        unit = _generate_service_unit("/usr/local/bin/patchbay")
        assert "ExecStart=/usr/local/bin/patchbay" in unit

    def test_has_restart_policy(self) -> None:
        unit = _generate_service_unit("patchbay")
    ...
    def test_includes_all_nvm_bins(self, tmp_path: Path) -> None:
        ...
        with patch("phoenix_patchbay.infra.service_linux.Path.home", return_value=tmp_path):
            unit = _generate_service_unit("patchbay")
...
class TestEnableLinger:
    @patch("phoenix_patchbay.infra.service_linux.subprocess.run")
    @patch("phoenix_patchbay.infra.service_linux.os.geteuid", return_value=0)
    ...
    @patch("phoenix_patchbay.infra.service_linux.subprocess.run")
    @patch("phoenix_patchbay.infra.service_linux.shutil.which", return_value="/usr/bin/sudo")
    @patch("phoenix_patchbay.infra.service_linux.os.geteuid", return_value=1000)
    ...
    @patch("phoenix_patchbay.infra.service_linux.subprocess.run")
    @patch("phoenix_patchbay.infra.service_linux.shutil.which", return_value=None)
    @patch("phoenix_patchbay.infra.service_linux.os.geteuid", return_value=1000)
    ...
    @patch(
        "phoenix_patchbay.infra.service_linux.subprocess.run",
...
class TestIsServiceAvailable:
    @patch("phoenix_patchbay.infra.service_linux.shutil.which", return_value="/usr/bin/systemctl")
...
class TestIsServiceRunning:
    @patch("phoenix_patchbay.infra.service_linux._run_systemctl")
    @patch("phoenix_patchbay.infra.service_linux.is_service_installed", return_value=True)
    @patch("phoenix_patchbay.infra.service_linux._has_systemd", return_value=True)
...
class TestStartService:
    @patch("phoenix_patchbay.infra.service_linux._run_systemctl")
    @patch("phoenix_patchbay.infra.service_linux._has_systemd", return_value=False)
    ...
    @patch("phoenix_patchbay.infra.service_linux._run_systemctl")
    @patch("phoenix_patchbay.infra.service_linux.is_service_installed", return_value=False)
    @patch("phoenix_patchbay.infra.service_linux._has_systemd", return_value=True)
    def test_start_not_installed(
        ...
        console.print.assert_called_with(
            "[dim]Service not installed. Run [bold]patchbay service install[/bold].[/dim]"
        )
...
class TestStopService:
    @patch("phoenix_patchbay.infra.service_linux._run_systemctl")
    @patch("phoenix_patchbay.infra.service_linux.is_service_running", return_value=True)
...
class TestUninstallService:
    ...
    @patch("phoenix_patchbay.infra.service_linux._run_systemctl")
    @patch("phoenix_patchbay.infra.service_linux._service_path")
    @patch("phoenix_patchbay.infra.service_linux.is_service_installed", return_value=True)
    @patch("phoenix_patchbay.infra.service_linux._has_systemd", return_value=True)
    def test_uninstall(
        self,
