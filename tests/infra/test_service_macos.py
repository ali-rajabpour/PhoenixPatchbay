...
from phoenix_patchbay.infra.service_macos import (
    _LABEL,
...
class TestGeneratePlistData:
    def test_contains_binary_path(self) -> None:
        data = _generate_plist_data("/usr/local/bin/patchbay")
        assert data["ProgramArguments"] == ["/usr/local/bin/patchbay"]

    def test_has_label(self) -> None:
        data = _generate_plist_data("patchbay")
        assert data["Label"] == "dev.phoenix-patchbay"
    ...
    def test_path_includes_all_nvm_bins(self, tmp_path: Path) -> None:
        ...
        with patch("phoenix_patchbay.infra.service_macos.Path.home", return_value=tmp_path):
            data = _generate_plist_data("patchbay")
    ...
    def test_generates_valid_plist(self) -> None:
        ...
        assert parsed["Label"] == "dev.phoenix-patchbay"
        assert parsed["RunAtLoad"] is True


class TestIsServiceInstalled:
    @patch("phoenix_patchbay.infra.service_macos._plist_path")
...
class TestIsServiceRunning:
    @patch("phoenix_patchbay.infra.service_macos.is_service_installed", return_value=False)
    def test_not_running_when_not_installed(self, _mock: MagicMock) -> None:
        assert is_service_running() is False

    @patch("phoenix_patchbay.infra.service_macos._run_launchctl")
    @patch("phoenix_patchbay.infra.service_macos.is_service_installed", return_value=True)
    def test_running_when_pid_present(self, _installed: MagicMock, mock_run: MagicMock) -> None:
        mock_run.return_value = make_completed(
            0,
            stdout='{\n\t"PID" = 12345;\n\t"Label" = "dev.phoenix-patchbay";\n};',
        )
...
class TestInstallService:
    @patch("phoenix_patchbay.infra.service_macos._run_launchctl")
    @patch("phoenix_patchbay.infra.service_macos.is_service_installed", return_value=False)
    @patch("phoenix_patchbay.infra.service_macos.is_service_available", return_value=True)
    @patch(
        "phoenix_patchbay.infra.service_macos.find_patchbay_binary", return_value="/usr/local/bin/patchbay"
    )
    @patch("phoenix_patchbay.infra.service_macos._plist_path")
    @patch("phoenix_patchbay.infra.service_macos.resolve_paths")
    def test_install_success(
        ...
        plist_file = tmp_path / "dev.phoenix-patchbay.plist"
        ...
        assert plist_data["Label"] == "dev.phoenix-patchbay"

    @patch("phoenix_patchbay.infra.service_macos.is_service_available", return_value=False)
    def test_install_fails_without_launchctl(self, _avail: MagicMock) -> None:
        console = MagicMock()
        assert install_service(console) is False

    @patch("phoenix_patchbay.infra.service_macos.is_service_available", return_value=True)
    @patch("phoenix_patchbay.infra.service_macos.find_patchbay_binary", return_value=None)
...
class TestStopService:
    @patch("phoenix_patchbay.infra.service_macos._run_launchctl")
    @patch("phoenix_patchbay.infra.service_macos.is_service_running", return_value=True)
    ...
    @patch("phoenix_patchbay.infra.service_macos.is_service_running", return_value=False)
...
class TestPrintServiceLogs:
    ...
    def test_shows_logs_from_file(
        ...
        logs_dir.mkdir()
        log_file = logs_dir / "patchbay_2026-02-22.log"
