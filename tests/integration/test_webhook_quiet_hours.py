...
from phoenix_patchbay.cli.param_resolver import RunOverrides
from phoenix_patchbay.config import AgentConfig, HeartbeatConfig, WebhookConfig
from phoenix_patchbay.webhook.manager import WebhookManager
from phoenix_patchbay.webhook.models import WebhookEntry
...
def _add_hook(mgr: WebhookManager, paths: PatchbayPaths, **overrides: Any) -> WebhookEntry:
    defaults: dict[str, Any] = {
        "id": "test-hook",
        "title": "Test Hook",
        "description": "Testing",
...
async def test_webhook_ignores_heartbeat_quiet_hours(tmp_path: Path) -> None:
    ...
    with patch("phoenix_patchbay.cron.execution.build_cmd", return_value=None):
