...
class TestBuildWelcomeText:
    def test_both_providers_authenticated(self) -> None:
        from phoenix_patchbay.messenger.telegram.welcome import build_welcome_text
        ...
        assert "Welcome to Phoenix Patchbay, Alice!" in text
    ...
    def test_user_name_present(self) -> None:
        ...
        assert "Welcome to Phoenix Patchbay, Zara!" in text

    def test_user_name_empty(self) -> None:
        from phoenix_patchbay.messenger.telegram.welcome import build_welcome_text

        text = build_welcome_text("", {}, _config())
        assert "Welcome to Phoenix Patchbay!" in text
        assert "Welcome to Phoenix Patchbay, " not in text
...
class TestBuildAuthBlock:
    def test_both_ok_mentions_both_providers(self) -> None:
        from phoenix_patchbay.messenger.telegram.welcome import _build_auth_block
...
class TestBuildWelcomeKeyboard:
    def test_returns_inline_keyboard_markup(self) -> None:
        ...
        from phoenix_patchbay.messenger.telegram.welcome import build_welcome_keyboard
...
class TestIsWelcomeCallback:
    ...
    def test_valid_welcome_data(self, data: str) -> None:
        from phoenix_patchbay.messenger.telegram.welcome import is_welcome_callback
...
class TestResolveWelcomeCallback:
    @pytest.mark.parametrize(
        ...
        [
            ("w:1", "phoenix patchbay"),
            ("w:2", "system"),
            ("w:3", "introduce"),
        ],
    )
    def test_known_keys_return_prompt(self, key: str, expected_substring: str) -> None:
        from phoenix_patchbay.messenger.telegram.welcome import resolve_welcome_callback
