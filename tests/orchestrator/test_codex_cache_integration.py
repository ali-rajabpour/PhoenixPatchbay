...
async def test_orchestrator_starts_cache_observer(mock_codex_cache: CodexModelCache) -> None:
    ...
    with (
        patch("phoenix_patchbay.orchestrator.observers.CodexCacheObserver", return_value=mock_observer),
        patch("phoenix_patchbay.orchestrator.lifecycle.resolve_paths"),
        patch("phoenix_patchbay.orchestrator.lifecycle.inject_runtime_environment"),
        patch(
...
async def test_orchestrator_passes_cache_to_observers(
    ...
    with (
        patch(
            "phoenix_patchbay.orchestrator.observers.CodexCacheObserver", return_value=mock_cache_observer
        ),
        patch("phoenix_patchbay.orchestrator.observers.CronObserver", mock_cron_class),
        patch("phoenix_patchbay.orchestrator.observers.WebhookObserver", mock_webhook_class),
        patch("phoenix_patchbay.orchestrator.lifecycle.resolve_paths"),
        patch("phoenix_patchbay.orchestrator.lifecycle.inject_runtime_environment"),
        patch(
            "phoenix_patchbay.cli.auth.check_all_auth",
