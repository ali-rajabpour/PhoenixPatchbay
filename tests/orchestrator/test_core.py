...
from phoenix_patchbay.errors import CLIError, CronError, SessionError, StreamError, WorkspaceError
from phoenix_patchbay.orchestrator.core import NamedSessionRequest, Orchestrator
...
async def test_clear_command(orch: Orchestrator) -> None:
    mock_kill = AsyncMock(return_value=0)
    object.__setattr__(orch._process_registry, "kill_by_chat_topic", mock_kill)
    result = await orch.handle_message(SessionKey(chat_id=1), "/clear")
    assert "cleared" in result.text.lower()
    mock_kill.assert_called_once_with(1, None)


async def test_clear_command_resets_only_active_provider_bucket(orch: Orchestrator) -> None:
    ...
    # The message no longer names a provider — /clear is about ending a task,
    # not about which backend was reset. The bucket assertions below are what
    # #82 was actually protecting.
...
async def test_create_with_authenticated_provider(
    workspace: tuple[PatchbayPaths, AgentConfig],
) -> None:
    ...
    with (
        patch(
            "phoenix_patchbay.orchestrator.lifecycle.resolve_paths",
            return_value=paths,
        ),
        patch(
            "phoenix_patchbay.cli.auth.check_all_auth",
            return_value={"claude": claude_auth, "codex": codex_auth},
        ),
        patch(
            "phoenix_patchbay.orchestrator.observers.watch_rule_files",
...
async def test_handle_heartbeat_delegates_to_flow(orch: Orchestrator) -> None:
    with patch(
        "phoenix_patchbay.orchestrator.core.heartbeat_flow",
        new_callable=AsyncMock,
...
async def test_submit_named_session_persists_transport_and_topic(orch: Orchestrator) -> None:
    ...
    with patch(
        "phoenix_patchbay.cli.param_resolver.resolve_cli_config",
...
async def test_handle_heartbeat_waits_for_shared_session_lock(orch: Orchestrator) -> None:
    ...
    with patch("phoenix_patchbay.orchestrator.core.heartbeat_flow", side_effect=_heartbeat_flow):
        async with lock:
...
async def test_submit_named_session_passes_effort(orch: Orchestrator) -> None:
    ...
    from phoenix_patchbay.session.named import NamedSession
    ...
    with patch("phoenix_patchbay.cli.param_resolver.resolve_cli_config", return_value=MagicMock()):
