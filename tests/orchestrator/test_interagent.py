...
from phoenix_patchbay.session.named import interagent_session_name
...
@pytest.fixture
def orch_ia(workspace: tuple[PatchbayPaths, AgentConfig]) -> Orchestrator:
...
class TestInteragentChatId:
    ...
    def test_returns_zero_when_no_users(self, workspace: tuple[PatchbayPaths, AgentConfig]) -> None:
        paths, config = workspace
...
def test_ask_agent_forwards_source_chat_topic(monkeypatch: pytest.MonkeyPatch) -> None:
    tool = _ROOT / "phoenix_patchbay/_home_defaults/workspace/tools/agent_tools/ask_agent.py"
    ...
    monkeypatch.setenv("PATCHBAY_AGENT_NAME", "main")
    monkeypatch.setenv("PATCHBAY_CHAT_ID", "777")
    monkeypatch.setenv("PATCHBAY_TOPIC_ID", "10")
    mod.main()
