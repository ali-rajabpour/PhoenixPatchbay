...
from phoenix_patchbay.config import MemoryCompactionConfig, MemoryFlushConfig
from phoenix_patchbay.orchestrator.memory_flush import MemoryFlusher
from phoenix_patchbay.session import SessionKey
from phoenix_patchbay.session.manager import ProviderSessionData, SessionData
...
def _make_paths(tmp_path: Path, mainmemory_lines: int = 0) -> PatchbayPaths:
    paths = PatchbayPaths(patchbay_home=tmp_path)
...
async def test_memory_flusher_falls_back_on_bad_prompt_placeholder(
    ...
    flusher.mark_boundary(key)
    with caplog.at_level("WARNING", logger="phoenix_patchbay.orchestrator.memory_flush"):
        await flusher.maybe_flush(key, session)
