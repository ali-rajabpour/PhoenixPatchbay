...
from phoenix_patchbay.config import reset_gemini_models, set_gemini_models
from phoenix_patchbay.orchestrator.core import Orchestrator
from phoenix_patchbay.orchestrator.flows import normal
...
from phoenix_patchbay.session.manager import ProviderSessionData
...
def _patch_auth(auth_map: dict[str, AuthResult]) -> Any:
    return patch(
        "phoenix_patchbay.orchestrator.selectors.model_selector.check_all_auth",
...
async def test_effort_selector_claude_shows_max(orch: Orchestrator) -> None:
    from phoenix_patchbay.orchestrator.selectors.model_selector import effort_selector_start
...
def test_effort_updated_locales_have_effort_placeholder() -> None:
    ...
    i18n_dir = Path(__file__).resolve().parents[2] / "phoenix_patchbay" / "i18n"
    found = 0
...
async def test_status_line_shows_session_effort_not_config(orch: Orchestrator) -> None:
    ...
    """
    from phoenix_patchbay.orchestrator.selectors.model_selector import _status_line
