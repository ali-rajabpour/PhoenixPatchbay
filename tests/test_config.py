...
def test_agent_config_docker_defaults() -> None:
    ...
    assert cfg.docker.image_name == "patchbay-sandbox"
...
def test_update_config_file_skips_unchanged_values(
    ...
    monkeypatch.setattr("phoenix_patchbay.infra.json_store.atomic_json_save", fake_atomic_json_save)
...
def test_update_config_file_does_not_log_values(
    ...
    from phoenix_patchbay.config import update_config_file

    config_path = tmp_path / "config.json"
    config_path.write_text("{}", encoding="utf-8")

    with caplog.at_level("DEBUG", logger="phoenix_patchbay.config"):
