...
from phoenix_patchbay.config import AgentConfig, WebhookConfig
from phoenix_patchbay.webhook.manager import WebhookManager
from phoenix_patchbay.webhook.models import WebhookEntry, WebhookResult, render_template
from phoenix_patchbay.webhook.observer import _SAFETY_END, _SAFETY_START, WebhookObserver
...
def _make_manager(paths: PatchbayPaths) -> WebhookManager:
...
class TestDispatchRouting:
    ...
    async def test_dispatch_wake_with_legacy_overrides_warns_and_dispatches(
        ...
        with caplog.at_level("WARNING", logger="phoenix_patchbay.webhook.observer"):
            result = await observer._dispatch("wake-hook", {"msg": "hello"})
