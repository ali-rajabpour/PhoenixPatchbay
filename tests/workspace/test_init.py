...
from phoenix_patchbay.workspace.init import init_workspace, inject_runtime_environment
...
def _setup_home_defaults(fw_root: Path) -> None:
    ...
    Mirrors the repo ``workspace/`` structure (1:1 copy of ~/.phoenix-patchbay/).
    """
    ws = fw_root / "workspace"

    # Top-level CLAUDE.md (patchbay home)
    ws.mkdir(parents=True)
    (ws / "CLAUDE.md").write_text("# Patchbay Home CLAUDE.md")
...
def test_media_tools_scripts_updated_on_reinit(tmp_path: Path) -> None:
    ...
    # Framework updates the template and patchbay restarts
    template = paths.home_defaults / "workspace" / "tools" / "media_tools" / "transcribe_audio.py"
    template.write_text("# v0.16.0 transcribe_audio with PATCHBAY_TRANSCRIBE_COMMAND")

    init_workspace(paths)

    assert deployed.read_text() == "# v0.16.0 transcribe_audio with PATCHBAY_TRANSCRIBE_COMMAND"
...
def test_zone2_user_modification_is_backed_up(
    ...
    with caplog.at_level(logging.WARNING, logger="phoenix_patchbay.workspace.init"):
        init_workspace(paths)
...
def test_patchbay_home_claude_md_overwritten(tmp_path: Path) -> None:
    """The patchbay_home/CLAUDE.md is Zone 2 (always overwritten)."""
    paths = _make_paths(tmp_path)
    init_workspace(paths)

    home_claude = paths.patchbay_home / "CLAUDE.md"
    assert home_claude.exists()
    assert home_claude.read_text() == "# Patchbay Home CLAUDE.md"
...
def test_inject_docker_notice(tmp_path: Path) -> None:
    paths = _make_paths(tmp_path)
    init_workspace(paths)
    inject_runtime_environment(paths, docker_container="patchbay-sandbox")
    content = (paths.workspace / "CLAUDE.md").read_text()
    assert "DOCKER CONTAINER" in content
    assert "patchbay-sandbox" in content
