...
from phoenix_patchbay.workspace.skill_sync import (
    _MANAGED_MARKER,
    _SYNCABLE_PROVIDERS,
    _clean_broken_links,
...
def test_canonical_patchbay_priority(tmp_path: Path) -> None:
    patchbay = {"sk": tmp_path / "patchbay" / "sk"}
    claude = {"sk": tmp_path / "claude" / "sk"}
    codex = {"sk": tmp_path / "codex" / "sk"}
    for d in (patchbay["sk"], claude["sk"], codex["sk"]):
        d.mkdir(parents=True)
    result = _resolve_canonical("sk", patchbay, claude, codex)
    assert result == patchbay["sk"]
...
def test_sync_claude_to_patchbay(tmp_path: Path) -> None:
    paths, claude_skills, _ = _setup_three_dirs(tmp_path)
    _make_skill(claude_skills, "from-claude")
    with patch("phoenix_patchbay.workspace.skill_sync._cli_skill_dirs") as mock:
...
def test_sync_codex_to_patchbay(tmp_path: Path) -> None:
...
def test_sync_patchbay_to_both(tmp_path: Path) -> None:
    paths, claude_skills, codex_skills = _setup_three_dirs(tmp_path)
    claude_skills.mkdir(parents=True, exist_ok=True)
    codex_skills.mkdir(parents=True, exist_ok=True)
    _make_skill(paths.skills_dir, "from-patchbay")
    with patch("phoenix_patchbay.workspace.skill_sync._cli_skill_dirs") as mock:
        mock.return_value = {"claude": claude_skills, "codex": codex_skills}
        sync_skills(paths)
    for d in (claude_skills, codex_skills):
        link = d / "from-patchbay"
        assert link.is_symlink()
        assert link.resolve() == (paths.skills_dir / "from-patchbay").resolve()


def test_sync_gemini_to_patchbay(tmp_path: Path) -> None:
...
def test_sync_patchbay_to_all_three(tmp_path: Path) -> None:
    paths, claude_skills, codex_skills = _setup_three_dirs(tmp_path)
    gemini_home = tmp_path / "fake_home" / ".gemini"
    gemini_home.mkdir(parents=True)
    gemini_skills = gemini_home / "skills"
...
def test_sync_external_symlink(tmp_path: Path) -> None:
    ...
    with patch("phoenix_patchbay.workspace.skill_sync._cli_skill_dirs") as mock:
        mock.return_value = {"claude": claude_skills, "codex": codex_skills}
        sync_skills(paths)
    patchbay_link = paths.skills_dir / "ext-skill"
    assert patchbay_link.is_symlink()
    assert patchbay_link.resolve() == external_real.resolve()
...
# ---------------------------------------------------------------------------
# Group 7: PatchbayPaths.skills_dir property
# ---------------------------------------------------------------------------
...
def test_permission_error_logged(tmp_path: Path, caplog: pytest.LogCaptureFixture) -> None:
    ...
    with (
        patch("phoenix_patchbay.workspace.skill_sync._cli_skill_dirs") as mock_dirs,
        patch("phoenix_patchbay.workspace.skill_sync._create_dir_link", side_effect=OSError("denied")),
    ):
...
def test_sync_preserves_external_symlink(tmp_path: Path) -> None:
    ...
    # Patchbay also has a skill with the same name
    _make_skill(paths.skills_dir, "my-skill")
...
def test_sync_replaces_internal_symlink(tmp_path: Path) -> None:
    """Symlinks pointing inside sync dirs (patchbay-managed) can be updated."""
    paths, claude_skills, codex_skills = _setup_three_dirs(tmp_path)
    claude_skills.mkdir(parents=True, exist_ok=True)
    codex_skills.mkdir(parents=True, exist_ok=True)
    ...
    # Patchbay now has a real skill with the same name (higher priority)
    ...
    # Claude's symlink should now point to patchbay (higher priority canonical)
...
# ---------------------------------------------------------------------------
# Group 12: cleanup_patchbay_links
# ---------------------------------------------------------------------------


def test_cleanup_removes_patchbay_links(tmp_path: Path) -> None:
    paths, claude_skills, codex_skills = _setup_three_dirs(tmp_path)
    claude_skills.mkdir(parents=True, exist_ok=True)
    codex_skills.mkdir(parents=True, exist_ok=True)

    # Simulate patchbay-created symlinks (target under patchbay skills dir)
    _make_skill(paths.skills_dir, "from-patchbay")
    (claude_skills / "from-patchbay").symlink_to(paths.skills_dir / "from-patchbay")
    (codex_skills / "from-patchbay").symlink_to(paths.skills_dir / "from-patchbay")

    with patch("phoenix_patchbay.workspace.skill_sync._cli_skill_dirs") as mock:
        mock.return_value = {"claude": claude_skills, "codex": codex_skills}
        removed = cleanup_patchbay_links(paths)

    assert removed == 2
    assert not (claude_skills / "from-patchbay").exists()
    assert not (codex_skills / "from-patchbay").exists()
...
def test_cli_skill_dirs_filters_disabled_provider(
    ...
    with patch("phoenix_patchbay.workspace.skill_sync.Path.home", return_value=fake_home):
...
def test_sync_skills_skipped_when_globally_disabled(tmp_path: Path) -> None:
    ...
    with patch("phoenix_patchbay.workspace.skill_sync._cli_skill_dirs") as mock_dirs:
        sync_skills(paths)
