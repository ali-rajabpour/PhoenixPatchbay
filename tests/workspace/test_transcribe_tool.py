...
def audio_module() -> Any:
    return _load_tool("_patchbay_test_transcribe_audio", "transcribe_audio.py")


@pytest.fixture
def video_module() -> Any:
    return _load_tool("_patchbay_test_process_video", "process_video.py")
...
def test_external_transcribe_unset_is_skip(
    ...
    monkeypatch.delenv("PATCHBAY_TRANSCRIBE_COMMAND", raising=False)
...
def test_external_transcribe_returns_transcript_json(
    ...
    monkeypatch.setenv("PATCHBAY_TRANSCRIBE_COMMAND", _python_command(script))
    audio = tmp_path / "a.ogg"
    audio.write_bytes(b"fake")
...
def test_external_transcribe_missing_binary_falls_through(
    ...
    monkeypatch.setenv("PATCHBAY_TRANSCRIBE_COMMAND", "/nonexistent/path/to/missing-bin")
    audio = tmp_path / "a.ogg"
    audio.write_bytes(b"fake")
...
def test_video_external_transcribe_returns_text(
    ...
    monkeypatch.setenv("PATCHBAY_VIDEO_TRANSCRIBE_COMMAND", _python_command(script))
...
def test_video_external_transcribe_unset_returns_none(
    ...
    monkeypatch.delenv("PATCHBAY_VIDEO_TRANSCRIBE_COMMAND", raising=False)
    audio = tmp_path / "x.ogg"
    audio.write_bytes(b"fake")
